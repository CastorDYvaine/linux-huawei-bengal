...
struct dmd_csiphy_error {
	csiphy_error_scene error_scene;
	char *error_scene_info;
};

const struct dmd_csiphy_error g_dmd_csiphy_error[CSIPHY_ERROR_SCENE_MAX] = {
	{ CSIPHY_LANE_0_OVERFLOW,        "(csiphy lane 0 over flow)", },
	{ CSIPHY_LANE_1_OVERFLOW,        "(csiphy lane 1 over flow)", },
	{ CSIPHY_LANE_2_OVERFLOW,        "(csiphy lane 2 over flow)", },
	{ CSIPHY_LANE_3_OVERFLOW,        "(csiphy lane 3 over flow)", },
	{ CSIPHY_TG_OVERFLOW,            "(csiphy tg over flow)", },
	{ CSIPHY_CPHY_EOT_RECEPTION,     "(csiphy cphy eot reception)", },
	{ CSIPHY_CPHY_SOT_RECEPTION,     "(csiphy cphy sot reception)", },
	{ CSIPHY_CPHY_PH_CRC,            "(csiphy cphy ph crc)", },
	{ CSIPHY_ERROR_CRC,              "(csiphy error crc)", },
	{ CSIPHY_ERROR_ECC,              "(csiphy error ecc)", },
	{ CSIPHY_MMAPPED_VC_DT,          "(csiphy mmapped vc dt)", },
	{ CSIPHY_UNMAPPED_VC_DT,         "(csiphy unmapped vc dt)", },
	{ CSIPHY_ERROR_STREAM_UNDERFLOW, "(csiphy error stream underflow)", },
	{ CSIPHY_UNBOUNDED_FRAME,        "(csiphy unbounded frame)", },
};

static csiphy_error_scene g_error_scene = CSIPHY_ERROR_SCENE_INVALID;
static int g_error_times = 0;

static inline void cam_tfe_hiview_handle(void)
{
	if (g_error_scene > CSIPHY_ERROR_SCENE_INVALID &&
		...
		if (!camkit_hiview_init(&cam_info)) {
			...
			if (strncpy_s(cam_info.extra_info.error_scene, sizeof(cam_info.extra_info.error_scene),
				g_dmd_csiphy_error[g_error_scene].error_scene_info, sizeof(cam_info.extra_info.error_scene) - 1)) {
