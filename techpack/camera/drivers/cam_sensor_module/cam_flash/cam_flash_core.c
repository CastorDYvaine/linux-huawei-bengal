	...
	uint64_t req_id)
{
	...
	int thermal_protect_value;
	int flash_read_offset = 0;

	CAM_DBG(CAM_FLASH, "req_id=%llu", req_id);

	thermal_protect_value = cam_torch_thermal_protect_get_value();
	if (thermal_protect_value == FLASH_STATUS_LOCKED) {
		CAM_ERR(CAM_FLASH, "thermal_protect_status locked");
		return 0;
	}
		...
		if (fctrl->apply_streamoff == true) {
			...
			list_for_each_entry(i2c_list,
				...
				if (i2c_list->op_code == CAM_SENSOR_I2C_READ_SEQ) {
					i2c_list->i2c_settings.read_buff += flash_read_offset;
					flash_read_offset += i2c_list->i2c_settings.size;
				}
				rc = cam_sensor_util_i2c_apply_setting
					(&(fctrl->io_master_info), i2c_list);
				if ((rc == -EAGAIN) &&
...
int cam_flash_i2c_pkt_parser(struct cam_flash_ctrl *fctrl, void *arg)
{
	...
	case CAM_FLASH_PACKET_OPCODE_INIT: {
		...
		for (i = 1; i < csl_packet->num_cmd_buf; i++) {
			...
			default:
				...
				if (io_cfg == NULL) {
					CAM_ERR(CAM_FLASH, "I/O config is invalid(NULL)");
					rc = -EINVAL;
					return rc;
				}
	...
	case CAM_FLASH_PACKET_OPCODE_SET_OFF_WITH_DELAY:
...
update_req_mgr:
	if (((csl_packet->header.op_code  & 0xFFFFF) ==
		...
		((csl_packet->header.op_code & 0xFFFFF) ==
		CAM_FLASH_PACKET_OPCODE_SET_OFF_WITH_DELAY)) {
		...
		} else if ((csl_packet->header.op_code & 0xFFFFF) ==
			CAM_FLASH_PACKET_OPCODE_SET_OFF_WITH_DELAY) {
			add_req.skip_before_applying = 1;
...
static void cam_flash_update_slave_info(
	struct cam_cmd_probe *probe_info, struct cam_flash_ctrl *fctrl)
{
	fctrl->probe_info.sensor_id_reg_addr = probe_info->reg_addr;
	fctrl->probe_info.sensor_id = probe_info->expected_data;
	fctrl->probe_info.sensor_id_mask = probe_info->data_mask;
	fctrl->flash_probe_addr_type =  probe_info->addr_type;
	fctrl->flash_probe_data_type =  probe_info->data_type;
	CAM_DBG(CAM_FLASH,
		"Flash probeID Addr: 0x%x chip_id: 0x%x ID_mask: 0x%x",
		fctrl->probe_info.sensor_id_reg_addr,
		fctrl->probe_info.sensor_id,
		fctrl->probe_info.sensor_id_mask);
}

static int32_t flash_handle_cmd_buffers_for_probe(void *cmd_buf,
	struct cam_flash_ctrl *fctrl,
	int32_t cmd_buf_num, uint32_t cmd_buf_length, size_t remain_len)
{
	int32_t rc = 0;
	struct cam_cmd_i2c_info *i2c_info = NULL;
	struct cam_cmd_probe *probe_info = NULL;

	switch (cmd_buf_num) {
	case CAM_PROBE_PKT_SLAVE_INFO_IDX:
		if (remain_len <
			(sizeof(struct cam_cmd_i2c_info) + sizeof(struct cam_cmd_probe))) {
			CAM_ERR(CAM_FLASH, "not enough buffer for cam_cmd_i2c_info");
			return -EINVAL;
		}
		i2c_info = (struct cam_cmd_i2c_info *)cmd_buf;
		rc = cam_flash_slaveInfo_pkt_parser(fctrl, cmd_buf, remain_len);
		if (rc < 0) {
			CAM_ERR(CAM_FLASH, "Failed in Updating the i2c Info");
			return rc;
		}
		probe_info = (struct cam_cmd_probe *)
			(cmd_buf + sizeof(struct cam_cmd_i2c_info));
		cam_flash_update_slave_info(probe_info, fctrl);
		cmd_buf = probe_info;
		break;
	case CAM_PROBE_PKT_NORMAL_POWER_IDX:
		rc = cam_sensor_update_power_settings(cmd_buf,
			cmd_buf_length, &fctrl->power_info, remain_len);
		if (rc < 0) {
			CAM_ERR(CAM_FLASH, "Failed in updating power settings");
			return rc;
		}
		break;
	default:
		CAM_ERR(CAM_FLASH, "Invalid command buffer");
		break;
	}
	return rc;
}

static int32_t flash_handle_packet(uint64_t handle,
	struct cam_cmd_buf_desc **cmd_desc)
{
	...
	rc = cam_mem_get_cpu_buf(handle, &packet, &len);
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "Failed to get the command Buffer");
		return -EINVAL;
	}

	pkt = (struct cam_packet *)packet;
	if (pkt == NULL) {
		CAM_ERR(CAM_FLASH, "packet pos is invalid");
		return -EINVAL;
	}

	if ((len < sizeof(struct cam_packet)) ||
		(pkt->cmd_buf_offset >= (len - sizeof(struct cam_packet)))) {
		CAM_ERR(CAM_FLASH, "Not enough buf provided");
		return -EINVAL;
	}

	*cmd_desc = (struct cam_cmd_buf_desc *)
		((uint32_t *)&pkt->payload + pkt->cmd_buf_offset / sizeof(uint32_t));
	if (*cmd_desc == NULL) {
		CAM_ERR(CAM_FLASH, "command descriptor pos is invalid");
		return -EINVAL;
	}
	if (pkt->num_cmd_buf != 2) { // 2 cmd buff, pSlaveInfoCmd and power squence
		CAM_ERR(CAM_FLASH, "Expected More Command Buffers : %d",
			 pkt->num_cmd_buf);
		return -EINVAL;
	}
	return rc;
...
int32_t flash_handle_mem_ptr(uint64_t handle,
	struct cam_flash_ctrl *fctrl)
{
	int rc;
	int idx;
	...
	rc = flash_handle_packet(handle, &cmd_desc);
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "Failed to handle packet");
		return rc;
	}

	for (idx = 0; idx <= CAM_PROBE_PKT_NORMAL_POWER_IDX; idx++) {
		if (!(cmd_desc[idx].length))
			continue;
		rc = cam_mem_get_cpu_buf(cmd_desc[idx].mem_handle, &cmd_buf1, &len);
		if (rc < 0) {
			CAM_ERR(CAM_FLASH, "Failed to parse the command Buffer Header");
			return rc;
		}
		if (cmd_desc[idx].offset >= len) {
			CAM_ERR(CAM_FLASH, "offset past length of buffer");
			return -EINVAL;
		}
		remain_len = len - cmd_desc[idx].offset;
		if (cmd_desc[idx].length > remain_len) {
			CAM_ERR(CAM_FLASH, "Not enough buffer provided for cmd");
			return -EINVAL;
		}
		cmd_buf = (uint32_t *)cmd_buf1;
		cmd_buf += cmd_desc[idx].offset / sizeof(uint32_t);

		rc = flash_handle_cmd_buffers_for_probe((void *)cmd_buf, fctrl,
			idx, cmd_desc[idx].length, remain_len);
		if (rc < 0) {
			CAM_ERR(CAM_FLASH, "Failed to parse the command Buffer Header");
			return rc;
		}
...
static uint16_t cam_flash_id_by_mask(struct cam_flash_ctrl *f_ctrl,
	uint32_t chipid)
{
	uint16_t flash_id = (uint16_t)(chipid & 0xFFFF);
	int16_t flash_id_mask = f_ctrl->probe_info.sensor_id_mask;

	if (!flash_id_mask)
		flash_id_mask = ~flash_id_mask;

	flash_id &= flash_id_mask;
	flash_id_mask &= -flash_id_mask;
	flash_id_mask -= 1;
	while (flash_id_mask) {
		flash_id_mask >>= 1;
...
int cam_flash_match_id(struct cam_flash_ctrl *f_ctrl)
{
	int rc = 0;
	uint32_t chipid = 0;
	struct msm_sensor_id_info_t *probe_info = NULL;

	if (!f_ctrl) {
		CAM_ERR(CAM_FLASH, "f_ctrl is NULL");
		return -EINVAL;
	}

	probe_info = &(f_ctrl->probe_info);
	if (!probe_info) {
		CAM_ERR(CAM_FLASH, "probe_info is NULL");
		return -EINVAL;
	}

	rc = camera_io_dev_read(
		&(f_ctrl->io_master_info), probe_info->sensor_id_reg_addr, &chipid,
		f_ctrl->flash_probe_addr_type, f_ctrl->flash_probe_data_type);
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "flash read matchid occur error, rc = 0x%x", rc);
		return -ENODEV;
	}
	CAM_DBG(CAM_FLASH, "read id: 0x%x expected id: 0x%x, mask: 0x%x",
		chipid, probe_info->sensor_id, f_ctrl->probe_info.sensor_id_mask);

	if (cam_flash_id_by_mask(f_ctrl, chipid) != probe_info->sensor_id) {
		CAM_WARN(CAM_FLASH, "match fail, read id: 0x%x expected id 0x%x:",
			chipid, probe_info->sensor_id);
		return -ENODEV;
	}
	CAM_INFO(CAM_FLASH, "flash match succ");
	return rc;
}
