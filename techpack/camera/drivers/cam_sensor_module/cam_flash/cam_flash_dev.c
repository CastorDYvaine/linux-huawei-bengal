...
#include "cam_torch_test.h"

static int cam_flash_handle_probe_cmd(struct cam_flash_ctrl *fctrl,
	struct cam_control *cmd)
{
	int rc = 0;

	CAM_INFO(CAM_FLASH, "recv flash probe cmd");
	if (cmd->handle_type == CAM_HANDLE_MEM_HANDLE) {
		rc = flash_handle_mem_ptr(cmd->handle, fctrl);
		if (rc < 0) {
			CAM_ERR(CAM_FLASH, "Get Buffer Handle Failed");
			return rc;
		}
	} else {
		CAM_ERR(CAM_FLASH, "Invalid Command Type: %d", cmd->handle_type);
		return (-EINVAL);
	}
	...
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "Fail in filling vreg params for PUP rc %d", rc);
		return rc;
	}
	...
	if (rc < 0) {
		CAM_ERR(CAM_FLASH, "Fail in filling vreg params for PDOWN rc %d", rc);
		return rc;
	}
	...
	rc = cam_flash_match_id(fctrl);
	if (rc < 0) {
		if (fctrl->func_tbl.power_ops(fctrl, false) < 0)
			CAM_ERR(CAM_FLASH, "fail in Sensor Power Down");
		msleep(5); /* 5ms delay for power up stable */
		return rc;
	}

	CAM_INFO(CAM_FLASH, "Probe success, ID addr:0x%x,sensor_id:0x%x",
		fctrl->probe_info.sensor_id_reg_addr, fctrl->probe_info.sensor_id);
		...
		void *arg, struct cam_flash_private_soc *soc_private)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		rc = cam_flash_handle_probe_cmd(fctrl, cmd);
		if (rc < 0)
			CAM_ERR(CAM_FLASH, "flash handle probe cmd failed");
	...
	case CAM_START_DEV: {
		...
		hw_flash_thermal_protect_set_fctrl(fctrl);
		break;
	}
	case CAM_STOP_DEV: {
...
static int32_t cam_flash_platform_probe(struct platform_device *pdev)
{
	...
	register_test_flashlight_data(&pdev->dev);

	fctrl->flash_state = CAM_FLASH_STATE_INIT;
	if (cam_torch_thermal_protect_classdev_register(pdev) < 0)
