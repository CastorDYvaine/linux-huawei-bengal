...
static struct mutex adapter_lock;
struct camera_name {
	...
	struct camera_io_master *io_master_info)
{
	...
	for (i = 0; i < debug_data->num; i++) {
		...
		if (rc < 0)
			return -1;
		debug_dbg("read iic reg 0x%x, val 0x%x", debug_data->data[i].reg,
				read_data->data[i].val);
...
module_init(debug_adapter_init);
module_exit(debug_adapter_exit);
MODULE_DESCRIPTION("CAM DEBUG driver");
