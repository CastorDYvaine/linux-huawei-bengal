...
enum reg_settings_type {
	NONE_SETTING = 0,
	INIT_SETTING,
	MATCH_ID_SETTING,
};

static inline void camkit_hiview_handle(int ret,
	int error_type, const char* error_scene, struct cam_sensor_ctrl_t* s_ctrl)
{
	if (ret == -EIO)
		cam_hiview_handle(error_type, s_ctrl, error_scene);
}
...
int32_t cam_handle_reg_settings_cmd_buffers(struct cam_sensor_ctrl_t *s_ctrl,
...
int32_t cam_handle_mem_ptr(uint64_t handle, struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (pkt->num_cmd_buf != CAM_PROBE_PKT_MAX_IDX) {
	...
	for (i = 0; i <= CAM_PROBE_PKT_NORMAL_POWER_IDX; i++) {
	...
	rc = cam_handle_cmd_buffers_for_hiz(s_ctrl, cmd_desc);
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR, "Failed to parse the hiz command Buffer");
		goto end;
	}
...
int32_t cam_handle_cmd_buffers_for_hiz(struct cam_sensor_ctrl_t *s_ctrl,
	struct cam_cmd_buf_desc *cmd_desc)
{
	int rc;
	int i;
	...
	uintptr_t cmd_buf1 = 0;
	struct i2c_settings_array *i2c_reg_settings = &s_ctrl->hiz_info.hiz_settings;
	for (i = CAM_PROBE_PKT_HIZ_TYPE_IDX; i <= CAM_PROBE_PKT_HIZ_POWER_IDX; i++) {
		if (!(cmd_desc[i].length))
			continue;
		rc = cam_mem_get_cpu_buf(cmd_desc[i].mem_handle, &cmd_buf1, &len);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "Failed to parse the cmd Buffer Header");
			return rc;
		}
		rc = -EINVAL;
		if (cmd_desc[i].offset >= len) {
			CAM_ERR(CAM_SENSOR, "offset past length of buffer");
			return rc;
		}
		remain_len = len - cmd_desc[i].offset;
		if (cmd_desc[i].length > remain_len) {
			CAM_ERR(CAM_SENSOR, "Not enough buffer provided for cmd");
			return rc;
		}
		cmd_buf = (uint32_t *)cmd_buf1;
		cmd_buf += cmd_desc[i].offset / 4;
		if (i == CAM_PROBE_PKT_HIZ_TYPE_IDX) {
			s_ctrl->hiz_info.hiz_type = *cmd_buf;
			CAM_INFO(CAM_SENSOR, "hiz_type=%u", s_ctrl->hiz_info.hiz_type);
		} else {
			if (s_ctrl->hiz_info.hiz_type != CAM_HIZ_NOT_NEED) {
				rc = cam_sensor_update_power_settings((void*)cmd_buf,
					cmd_desc[i].length, &s_ctrl->hiz_info.hiz_power_info,
					remain_len);
				if (rc < 0) {
					CAM_ERR(CAM_SENSOR, "Failed update hiz power settings");
					return rc;
				}
			}
		}
	...
	i2c_reg_settings->is_settings_valid = 1;
	rc = cam_sensor_i2c_command_parser(
		&s_ctrl->io_master_info, i2c_reg_settings,
		&cmd_desc[CAM_PROBE_PKT_HIZ_I2C_SETTING_IDX], 1, NULL);
	if (rc < 0)
		CAM_ERR(CAM_SENSOR, "parsing hiz i2c setting failed");
	return rc;
}
...
void cam_sensor_shutdown(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	kfree(s_ctrl->hiz_info.hiz_power_info.power_setting);
	kfree(s_ctrl->hiz_info.hiz_power_info.power_down_setting);
	s_ctrl->hiz_info.hiz_power_info.power_setting = NULL;
	s_ctrl->hiz_info.hiz_power_info.power_down_setting = NULL;
	s_ctrl->hiz_info.hiz_power_info.power_setting_size = 0;
	s_ctrl->hiz_info.hiz_power_info.power_down_setting_size = 0;
...
static int32_t cam_sensor_apply_reg_settings(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (slave_info->sensor_probe_info.sensor_reg_settings_type != NONE_SETTING) {
...
/*
 * Customer dump reg info when sof freeze.
 */
static int cam_sensor_dump_reg(const uint64_t handle,
	struct camera_io_master *io_master_info)
{
	int rc;
	struct cam_dump_reg_info reg_info;
	reg_info.reg_addr = 0;
	reg_info.addr_type = 0;
	reg_info.data_type = 0;
	reg_info.reg_count = 0;

	if (io_master_info == NULL) {
		CAM_ERR(CAM_SENSOR, "HwCamDump, io_master_info is NULL");
		return -EINVAL;
	}

	/* user to kernel */
	if (copy_from_user(&reg_info, u64_to_user_ptr(handle), sizeof(reg_info))) {
		CAM_ERR(CAM_SENSOR, "HwCamDump, Failed Copy to User");
		return -EFAULT;
	}

	if (reg_info.reg_count < 1) {
		CAM_ERR(CAM_SENSOR, "HwCamDump, reg_count err");
		rc = -EFAULT;
	} else if (reg_info.reg_count == 1) {
		uint32_t value = 0;
		rc = camera_io_dev_read(io_master_info,
			reg_info.reg_addr, &value, reg_info.addr_type, reg_info.data_type);
		CAM_INFO(CAM_SENSOR,
			"HwCamDump, Addr:0x%04x, value:0x%04x, addr_type %d, data_type %d, rc %d",
			reg_info.reg_addr, value, reg_info.addr_type, reg_info.data_type, rc);
	} else {
		int32_t num_bytes = reg_info.data_type * sizeof(uint8_t) * reg_info.reg_count;
		uint8_t *value = kzalloc(num_bytes + 1, GFP_KERNEL);
		if (value == NULL) {
			CAM_ERR(CAM_SENSOR, "HwCamDump, kzalloc fail!");
			return -EFAULT;
		}
		rc = camera_io_dev_read_seq(io_master_info, reg_info.reg_addr, value,
			reg_info.addr_type, reg_info.data_type, num_bytes);
		CAM_INFO(CAM_SENSOR,
			"HwCamDump, Addr:0x%04x, addr_type %d, data_type %d, reg_count %d, rc %d",
			reg_info.reg_addr, reg_info.addr_type, reg_info.data_type,
			reg_info.reg_count, rc);
		if (!rc) {
			int i;
			if (reg_info.data_type == CAMERA_SENSOR_I2C_TYPE_BYTE)
				for (i = 0; i < reg_info.reg_count; i++)
					CAM_INFO(CAM_SENSOR,
						"[%d/%d] Addr:0x%04x, value:0x%02x",
						i + 1, reg_info.reg_count,
						reg_info.reg_addr + i * reg_info.data_type, value[i]);
			else if (reg_info.data_type == CAMERA_SENSOR_I2C_TYPE_WORD)
				for (i = 0; i < reg_info.reg_count; i++)
					CAM_INFO(CAM_SENSOR,
						"[%d/%d] Addr:0x%04x, value:0x%02x%02x",
						i + 1, reg_info.reg_count,
						reg_info.reg_addr + i * reg_info.data_type,
						value[i * reg_info.data_type],
						value[i * reg_info.data_type + 1]); /* next */
			else if (reg_info.data_type == CAMERA_SENSOR_I2C_TYPE_3B)
				for (i = 0; i < reg_info.reg_count; i++)
					CAM_INFO(CAM_SENSOR,
						"[%d/%d] Addr:0x%04x, value:0x%02x%02x%02x",
						i + 1, reg_info.reg_count,
						reg_info.reg_addr + i * reg_info.data_type,
						value[i * reg_info.data_type],
						value[i * reg_info.data_type + 1],  /* next */
						value[i * reg_info.data_type + 2]); /* next */
			else if (reg_info.data_type == CAMERA_SENSOR_I2C_TYPE_DWORD)
				for (i = 0; i < reg_info.reg_count; i++)
					CAM_INFO(CAM_SENSOR,
						"[%d/%d] Addr:0x%04x, value:0x%02x%02x%02x%02x",
						i + 1, reg_info.reg_count,
						reg_info.reg_addr + i * reg_info.data_type,
						value[i * reg_info.data_type],
						value[i * reg_info.data_type + 1],  /* next */
						value[i * reg_info.data_type + 2],  /* next */
						value[i * reg_info.data_type + 3]); /* next */
		}
		kfree(value);
		value = NULL;
	}
	...
	void *arg)
{
	...
	case CAM_ACQUIRE_HIZ_DEV: {
		struct cam_sensor_acquire_dev sensor_acq_dev;
		struct cam_create_dev_hdl bridge_params;

		check_camera_btb_gpio_info(s_ctrl);
		if (cmd->op_code == CAM_ACQUIRE_HIZ_DEV &&
			s_ctrl->hiz_info.hiz_type == CAM_HIZ_NOT_NEED)
			break;
		...
		if (cmd->op_code == CAM_ACQUIRE_HIZ_DEV)
			rc = cam_sensor_hiz_power_up(s_ctrl);
		else
			rc = cam_sensor_power_up(s_ctrl);
		if (rc < 0) {
	...
	case CAM_RELEASE_HIZ_DEV: {
		if (cmd->op_code == CAM_RELEASE_HIZ_DEV &&
			s_ctrl->hiz_info.hiz_type == CAM_HIZ_NOT_NEED)
			break;
		...
		if (cmd->op_code == CAM_RELEASE_HIZ_DEV)
			rc = cam_sensor_hiz_power_down(s_ctrl);
		else
			rc = cam_sensor_power_down(s_ctrl);
		if (rc < 0) {
	...
	case CAM_START_DEV: {
		...
		if (s_ctrl->i2c_data.streamon_settings.is_settings_valid &&
			...
			if (rc < 0) {
				camkit_hiview_handle(rc, SENSOR_I2C_ERR,
					"(set stream on setting fail)", s_ctrl);
				CAM_ERR(CAM_SENSOR,
	...
	case CAM_STOP_DEV: {
		...
		if (s_ctrl->i2c_data.streamoff_settings.is_settings_valid &&
			...
			if (rc < 0) {
				camkit_hiview_handle(rc, SENSOR_I2C_ERR,
					"(set stream off setting fail)", s_ctrl);
				CAM_ERR(CAM_SENSOR,
	...
	case CAM_CONFIG_DEV: {
		...
		break;
	case CAM_DUMP_CUSTOMER: { /* Customer dump reg info when sof freeze */
		CAM_INFO(CAM_SENSOR,
			"HwCamDump, freezing, dumping happen, print the dump reg info+");
		rc = cam_sensor_dump_reg(cmd->handle, &(s_ctrl->io_master_info));
		CAM_INFO(CAM_SENSOR,
			"HwCamDump, freezing, dumping happen, print the dump reg info-");
		break;
	}
...
int cam_sensor_hiz_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	int rc;
	struct cam_sensor_power_ctrl_t *power_info = NULL;
	struct cam_hw_soc_info *soc_info = &s_ctrl->soc_info;

	if (s_ctrl->hiz_info.hiz_type == CAM_HIZ_WRITE_I2C ||
		s_ctrl->hiz_info.hiz_type == CAM_HIZ_WRITE_I2C_DEBUG) {
		power_info = &s_ctrl->sensordata->power_info;
	} else {
		power_info = &s_ctrl->hiz_info.hiz_power_info;
		power_info->dev = s_ctrl->sensordata->power_info.dev;
		power_info->gpio_num_info = s_ctrl->sensordata->power_info.gpio_num_info;
		rc = memcpy_s(&(power_info->pinctrl_info),
				sizeof(power_info->pinctrl_info),
				&(s_ctrl->sensordata->power_info.pinctrl_info),
				sizeof(s_ctrl->sensordata->power_info.pinctrl_info));
		if (rc != EOK) {
			CAM_ERR(CAM_SENSOR, "memcpy_s failed:%d", rc);
			return rc;
		}
	}
	...
	if (s_ctrl->hiz_info.hiz_type == CAM_HIZ_WRITE_I2C ||
		...
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "cci_init failed: rc: %d", rc);
			cam_sensor_hiz_power_down(s_ctrl);
			return rc;
		}

		rc = cam_sensor_apply_settings_array(s_ctrl,
			&s_ctrl->hiz_info.hiz_settings);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "Cannot apply hiz settings");
			cam_sensor_hiz_power_down(s_ctrl);
			camera_io_release(&(s_ctrl->io_master_info));
			return rc;
		}

		if (s_ctrl->hiz_info.hiz_type == CAM_HIZ_WRITE_I2C_DEBUG)
			rc = cam_sensor_i2c_read_back(s_ctrl,
				&s_ctrl->hiz_info.hiz_settings);
	}
	return rc;
}

int cam_sensor_hiz_power_down(struct cam_sensor_ctrl_t *s_ctrl)
{
	struct cam_sensor_power_ctrl_t *power_info = NULL;
...
int32_t cam_sensor_i2c_read_back(struct cam_sensor_ctrl_t *s_ctrl,
	struct i2c_settings_array *i2c_settings)
{
	...
	if (s_ctrl == NULL || i2c_settings == NULL) {
		CAM_ERR(CAM_SENSOR, "failed: null ptr");
		return -EINVAL;
	}

	list_for_each_entry(i2c_list, &(i2c_settings->list_head), list) {
		for (cnt = 0; cnt < (i2c_list->i2c_settings.size); cnt++) {
			...
			CAM_INFO(CAM_SENSOR,
				"reg_addr:0x%x reg_data:0x%x read_data:0x%x",
				reg_setting->reg_addr, reg_setting->reg_data, read_data);
		}
	}
	return rc;
}
