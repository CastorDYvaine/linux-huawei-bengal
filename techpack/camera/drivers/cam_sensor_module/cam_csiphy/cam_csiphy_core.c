	...
	struct cam_config_dev_cmd *cfg_dev)
{
	...
	csiphy_dev->csiphy_info.csiphy_3phase =
		cam_cmd_csiphy_info->csiphy_3phase;
	if (csiphy_dev->csiphy_info.lane_cnt == 4)
	...
	while (lane_mask) {
		...
		for (i = 0; i < cfg_size; i++) {
			...
			case CSIPHY_DNP_PARAMS:
				if ((lane_cnt == 4) && (csiphy_dev->csiphy_info.csiphy_3phase == 0)) {
					CAM_DBG(CAM_CSIPHY, "update addr=0x%x, data=0x%x",
						reg_array[lane_pos][i].reg_addr, reg_array[lane_pos][i].reg_data);
					cam_io_w_mb(0, csiphybase + reg_array[lane_pos][i].reg_addr);
				}
			break;
			default:
