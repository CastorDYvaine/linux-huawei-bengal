	...
	struct cam_buf_io_cfg *io_cfg)
{
	...
	if (rc) {
		...
		*offset = cmd_i2c_continuous_rd->header.count;
		*list = &(i2c_list->list);
	}
	...
	struct i2c_settings_list *i2c_list)
{
	...
	case CAM_SENSOR_I2C_READ_SEQ: {
		rc = camera_io_dev_read_seq(
			io_master_info,
			i2c_list->i2c_settings.reg_setting[0].reg_addr,
			i2c_list->i2c_settings.read_buff,
			i2c_list->i2c_settings.addr_type,
			i2c_list->i2c_settings.data_type,
			i2c_list->i2c_settings.size);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"Failed to seq read I2C settings: %d",
				rc);
			return rc;
		}

		usleep_time = i2c_list->i2c_settings.reg_setting[i2c_list->i2c_settings.size - 1].delay;
		if (usleep_time > 0)
			usleep_range(usleep_time, usleep_time + 1000);

		if (i2c_list->i2c_settings.delay > 20) {
			msleep(i2c_list->i2c_settings.delay);
		} else if (i2c_list->i2c_settings.delay > 0) {
			usleep_range(i2c_list->i2c_settings.delay * 1000,
				i2c_list->i2c_settings.delay * 1000 + 1000);
		}
	...
	struct msm_camera_gpio_num_info **pgpio_num_info)
{
	...
	*pgpio_num_info = kzalloc(sizeof(struct msm_camera_gpio_num_info),
	...
	rc = of_property_read_u32(of_node, "gpio-custom-vana", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read gpio-custom-vana failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "gpio-custom-vana invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "gpio-custom-vana %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VANA]);
	}
	...
	rc = of_property_read_u32(of_node, "gpio-custom-vio", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read gpio-custom-vio failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "gpio-custom-vio invalid %d", val);
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "gpio-custom-vio %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VIO]);
	}
	...
	rc = of_property_read_u32(of_node, "gpio-custom-vaf", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read gpio-custom-vaf failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "gpio-custom-vaf invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "gpio-custom-vaf %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VAF]);
	}
