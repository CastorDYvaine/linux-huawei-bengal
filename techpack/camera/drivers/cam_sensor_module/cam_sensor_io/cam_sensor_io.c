	...
	uint32_t delay_ms)
{
	...
	if (rc) {
		cam_i2c_error_hiview_handle(io_master_info, "(io dev poll failed)");
	...
	enum camera_sensor_i2c_type data_type)
{
	...
	if (rc) {
		cam_i2c_error_hiview_handle(io_master_info, "(io dev read failed)");
	...
	enum camera_sensor_i2c_type data_type, int32_t num_bytes)
{
	...
	if (io_master_info->master_type == CCI_MASTER) {
		if (io_master_info->insensor_type == CAM_SENSOR_OTP_HYNIX) {
			rc = cam_camera_cci_i2c_read_hynix(io_master_info->cci_client,
	...
	if (rc) {
		cam_i2c_error_hiview_handle(io_master_info, "(io dev read seq failed)");
	...
	struct cam_sensor_i2c_reg_setting *write_setting)
{
	...
	if (rc) {
		cam_i2c_error_hiview_handle(io_master_info, "(io dev write failed)");
	...
	uint8_t cam_sensor_i2c_write_flag)
{
	...
	if (rc) {
		cam_i2c_error_hiview_handle(io_master_info, "(io dev write continuous failed)");
