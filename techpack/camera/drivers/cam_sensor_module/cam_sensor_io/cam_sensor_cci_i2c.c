...
int32_t cam_camera_cci_i2c_read_hynix(struct cam_sensor_cci_client *cci_client,
	uint32_t addr, uint8_t *data,
	enum camera_sensor_i2c_type addr_type,
	enum camera_sensor_i2c_type data_type,
	uint32_t num_byte)
{
	...
	cci_ctrl.status = -EFAULT;
	CAM_DBG(CAM_SENSOR, "addr = 0x%x, rc = %d, num_byte = %d", addr, rc, num_byte);
	for (i = 0; i < num_byte; i++) {
		rc = v4l2_subdev_call(cci_client->cci_subdev,
			core, ioctl, VIDIOC_MSM_CCI_CFG, &cci_ctrl);
		rc = cci_ctrl.status;
		data[i] = buf[0];
		CAM_DBG(CAM_SENSOR, "Byte %d: Data: 0x%x, buf: 0x%x\n", i, data[i], buf[0]);
	}
	kfree(buf);
	return rc;
}
