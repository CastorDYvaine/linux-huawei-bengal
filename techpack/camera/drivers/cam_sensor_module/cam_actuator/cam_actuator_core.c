...
static int32_t cam_actuator_power_down(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	cam_actuator_set_suspend(a_ctrl);
	soc_private =
		(struct cam_actuator_soc_private *)a_ctrl->soc_info.soc_private;
	power_info = &soc_private->power_info;
	...
	case CAM_ACTUATOR_PACKET_OPCODE_INIT:
		...
		for (i = 0; i < csl_packet->num_cmd_buf; i++) {
			...
			default:
				...
				if (i == ACTUATOR_INIT_PKT_DEINIT_SETTING_IDX) {
					cam_actuator_update_deinit_setting(a_ctrl, cmd_desc, i);
					break;
				}
...
static void cam_actuator_update_slave_info(
	struct cam_cmd_probe *probe_info, struct cam_actuator_ctrl_t *a_ctrl)
{
	a_ctrl->probe_info.sensor_id_reg_addr = probe_info->reg_addr;
	a_ctrl->probe_info.sensor_id = probe_info->expected_data;
	a_ctrl->probe_info.sensor_id_mask = probe_info->data_mask;
	a_ctrl->actuator_probe_addr_type =  probe_info->addr_type;
	a_ctrl->actuator_probe_data_type =  probe_info->data_type;
	CAM_ERR(CAM_FLASH,
		"actuator probeID Addr: 0x%x chip_id: 0x%x ID_mask: 0x%x",
		a_ctrl->probe_info.sensor_id_reg_addr,
		a_ctrl->probe_info.sensor_id,
		a_ctrl->probe_info.sensor_id_mask);
}

static int32_t actuator_handle_cmd_buffers_for_probe(void *cmd_buf,
	struct cam_actuator_ctrl_t *a_ctrl,
	int32_t cmd_buf_num, uint32_t cmd_buf_length, size_t remain_len)
{
	...
	power_info = &soc_private->power_info;
	switch (cmd_buf_num) {
	case ACTUATOR_PROB_PKT_SLAVE_INFO_IDX:
		if (remain_len <
		...
		i2c_info = (struct cam_cmd_i2c_info *)cmd_buf;
		rc = cam_actuator_slaveInfo_pkt_parser(a_ctrl, cmd_buf, remain_len);
		if (rc < 0) {
		...
		cam_actuator_update_slave_info(probe_info, a_ctrl);
		cmd_buf = probe_info;
		break;
	case ACTUATOR_PROB_PKT_NORMAL_POWER_IDX:
		rc = cam_sensor_update_power_settings(cmd_buf,
			cmd_buf_length, power_info, remain_len);
		if (rc < 0) {
...
int32_t actuator_handle_mem_ptr(uint64_t handle,
	struct cam_actuator_ctrl_t *a_ctrl)
{
	int rc = 0;
	int idx;
	...
	for (idx = 0; idx <= pkt->num_cmd_buf; idx++) {
		...
		rc = actuator_handle_cmd_buffers_for_probe((void *)cmd_buf, a_ctrl,
			idx, cmd_desc[idx].length, remain_len);
		if (rc < 0) {
			CAM_ERR(CAM_FLASH, "actuator_handle_cmd_buffers_for_probe fail ");
			return rc;
		}
		if (idx == ACTUATOR_PROB_PKT_DEINIT_SETTING_IDX) {
			rc = cam_actuator_update_deinit_setting(a_ctrl, cmd_desc, idx);
			if (rc < 0) {
				CAM_ERR(CAM_FLASH, "Failed in updating pd deinit settings");
				return rc;
			}
		}
	}
...
static int cam_actuator_handle_probe_cmd(struct cam_actuator_ctrl_t *a_ctrl,
	struct cam_control *cmd)
{
	int rc = 0;

	CAM_INFO(CAM_ACTUATOR, "recv actuator probe cmd");
	if (cmd->handle_type == CAM_HANDLE_MEM_HANDLE) {
		rc = actuator_handle_mem_ptr(cmd->handle, a_ctrl);
		if (rc < 0) {
			CAM_ERR(CAM_ACTUATOR, "Get Buffer Handle Failed");
			return rc;
		}
	} else {
		CAM_ERR(CAM_ACTUATOR, "Invalid Command Type: %d", cmd->handle_type);
		return (-EINVAL);
	}

	rc = cam_actuator_power_up(a_ctrl);
	if (rc < 0) {
		CAM_ERR(CAM_ACTUATOR, "fail in Sensor Power up");
		return rc;
	}

	rc = cam_actuator_power_down(a_ctrl);
	if (rc < 0)
		CAM_ERR(CAM_ACTUATOR, "fail in Sensor Power Down");
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		rc = cam_actuator_handle_probe_cmd(a_ctrl, cmd);
		if (rc < 0)
			CAM_INFO(CAM_ACTUATOR, "CAM_SENSOR_PROBE_CMD fail");
		break;
	}
	...
	struct cam_cmd_buf_desc *cmd_desc, uint32_t idx)
{
	...
	struct i2c_settings_array *i2c_reg_settings = NULL;
	if (!a_ctrl || !cmd_desc) {
		CAM_ERR(CAM_ACTUATOR, "ptr is NULL");
		return -EINVAL;
	}

	i2c_reg_settings = &a_ctrl->i2c_data.deinit_settings;
	delete_request(i2c_reg_settings);
	i2c_reg_settings->request_id = 0;
	i2c_reg_settings->is_settings_valid = 1;
	rc = cam_sensor_i2c_command_parser(
		&a_ctrl->io_master_info, i2c_reg_settings,
		&cmd_desc[idx], 1, NULL);
	if (rc < 0)
		CAM_ERR(CAM_SENSOR, "parsing deinit setting failed");
...
void cam_actuator_set_suspend(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (a_ctrl->i2c_data.deinit_settings.is_settings_valid == 1) {
		rc = cam_actuator_apply_settings(a_ctrl,
			&a_ctrl->i2c_data.deinit_settings);
		if (rc < 0)
			CAM_ERR(CAM_ACTUATOR, "Cannot apply deinit settings");

		rc = delete_request(&a_ctrl->i2c_data.deinit_settings);
		if (rc < 0)
			CAM_WARN(CAM_ACTUATOR, "Failed delete deinit settings");
	}
}
