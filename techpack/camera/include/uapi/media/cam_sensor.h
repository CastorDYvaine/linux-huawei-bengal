...
#define CAM_ACQUIRE_HIZ_DEV    (CAM_COMMON_OPCODE_MAX + 2)
#define CAM_RELEASE_HIZ_DEV    (CAM_COMMON_OPCODE_MAX + 3)
...
struct custom_module_version_info {
	uint8_t module_version_flag;
	uint16_t module_version_addr;
	uint16_t module_version_data;
} __attribute__((packed));

struct custom_sensor_probe_info {
	/* probe sensor info */
	uint8_t sensor_reg_settings_type;
} __attribute__((packed));

struct custom_module_probe_info {
	/* probe module info */
	uint8_t module_code_support;
	uint8_t module_code_data_type;
	uint8_t module_code_addr_type;
	uint16_t eeprom_slave_addr;
	uint16_t module_code_data;
	uint16_t module_code_addr;
...
enum cam_probe_packet_idx_t {
	...
	CAM_PROBE_PKT_HIZ_TYPE_IDX,
	CAM_PROBE_PKT_HIZ_POWER_IDX,
	CAM_PROBE_PKT_HIZ_I2C_SETTING_IDX,
