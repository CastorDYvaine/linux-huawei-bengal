...
static bool chose_custom_channel_map_in_mode1(int port_idx, int idx)
{
	unsigned int pa_num;
	bool get_pa_num_ret = hw_get_smartpa_num(&pa_num);
	if (get_pa_num_ret && pa_num > 2) {
		if (port_idx == IDX_SLIMBUS_7_TX &&
			this_adm.get_hal_ec_en &&
			this_adm.is_voip_ec_enable &&
			this_adm.ec_special_handle)
			return true;
	} else {
		if ((port_idx == IDX_AFE_PORT_ID_TX_CODEC_DMA_TX_4 ||
			port_idx == IDX_SLIMBUS_7_TX) &&
			this_adm.get_hal_ec_en &&
			this_adm.is_voip_ec_enable)
			return true;
	}
		...
		int path, int channel_mode, int port_idx)
{
	...
	if ((ep_payload->dev_num_channel > 2) &&
		...
		if (channel_mode == 1) {
			...
			if (chose_custom_channel_map_in_mode1(port_idx, idx)) {
