...
static int g_aw_tx_port_id = AFE_PORT_ID_SECONDARY_MI2S_TX;
static int g_aw_rx_port_id = AFE_PORT_ID_SECONDARY_MI2S_RX;
	...
	if (ret > 0) {
		if (wait) {
			...
			if (!ret) {
				...
				if (strcmp(adsp_err_get_err_str(atomic_read(
					&this_afe.status)), "ADSP_EBADPARAM")) {
					adsp_err_cnt++;
				}
			} else {
				ret = 0;
		...
		if (resp_time_out_cnt >= resp_timeout_max || adsp_err_cnt >= resp_timeout_max) {
			pr_err("%s: adsp respone timeout cnt %d or return err cnt %d, need reset\n",
				__func__, resp_time_out_cnt, adsp_err_cnt);
...
{
        union afe_spkr_prot_config config;
        int32_t port_id = g_aw_tx_port_id;//AFE_PORT_ID_AWDSP_TX;
