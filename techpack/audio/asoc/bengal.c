...
extern bool adm_get_viop_ec_enable(void);
...
static int huawei_custom_voip_ec_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	if (adm_get_viop_ec_enable())
		ucontrol->value.integer.value[0] = 1;
	else
		ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int huawei_custom_voip_ec_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	bool is_voip_ec_enable = ucontrol->value.integer.value[0] ? true : false;
	adm_set_viop_ec_enable(is_voip_ec_enable);
	return 0;
}
...
static const struct snd_kcontrol_new msm_common_snd_controls[] = {
	...
	SOC_SINGLE_EXT("CUSTOM_VOIP_EC Enable", SND_SOC_NOPM, 0, 1, 0,
		huawei_custom_voip_ec_get, huawei_custom_voip_ec_put),
};
...
#ifdef CONFIG_HWEVEXT_EXTERN_ADC
static bool is_use_extern_adc(struct snd_soc_pcm_runtime *rtd)
{
	if (!rtd)
		return false;
	...
	return of_property_read_bool(rtd->card->dev->of_node,
...
#ifdef CONFIG_SUPPORT_AUDIO_TDM
	{/* hw:x,35 */
...
static struct snd_soc_dai_link msm_mi2s_qcom_be_dai_links[] = {
	{
...
static struct snd_soc_dai_link msm_bengal_dai_links[
			...
			ARRAY_SIZE(msm_mi2s_qcom_be_dai_links) +
	...
	if (!strcmp(match->data, "codec")) {
		...
		if (rc) {
			...
			if (mi2s_audio_intf) {
				...
				memcpy(msm_bengal_dai_links + total_links,
					msm_mi2s_qcom_be_dai_links,
					sizeof(msm_mi2s_qcom_be_dai_links));
				total_links +=
					ARRAY_SIZE(msm_mi2s_qcom_be_dai_links);
				}
			}
...
static int msm_aux_codec_init(struct snd_soc_component *component)
{
	...
	if (pdata->wcd_disabled) {
		dev_info(component->dev, "%s: codec is disable\n",
			__func__);
		return 0;
	}
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "qcom,wcd-disabled",
				&pdata->wcd_disabled))
		pdata->wcd_disabled = 0;

	if (of_property_read_bool(pdev->dev.of_node, "qcom,msm-mbhc-btn-threshold")) {
		pdata->rewrite_btn_threshold = true;
		ret = msm_parse_mbhc_btn_threshold(&pdev->dev, "qcom,msm-mbhc-btn-threshold", pdata);
		if (ret < 0) {
	...
	if (of_property_read_bool(pdev->dev.of_node, "qcom,msm-mbhc-moisture-disable")) {
		wcd_mbhc_cfg.moisture_en = false;
		dev_info(&pdev->dev, "%s: disable mbhc moisture detection\n", __func__);
	}

	if (of_property_read_bool(pdev->dev.of_node, "qcom,msm-mbhc-moisture-duty-cycle-disable")) {
		wcd_mbhc_cfg.moisture_duty_cycle_en = false;
		dev_info(&pdev->dev, "%s: disable mbhc moisture duty cycle\n", __func__);
	}
