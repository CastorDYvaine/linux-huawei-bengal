...
static int wcd937x_voip_ec_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	...
	ucontrol->value.integer.value[0] = wcd937x->is_voip_ec_enable;

	return 0;
}

static int wcd937x_voip_ec_put(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
				snd_soc_kcontrol_component(kcontrol);
	...
	wcd937x->is_voip_ec_enable = ucontrol->value.integer.value[0];
	adm_set_viop_ec_enable(wcd937x->is_voip_ec_enable);
...
static const struct snd_kcontrol_new wcd937x_snd_controls[] = {
	...
	SOC_SINGLE_EXT("VOIP_EC Enable", SND_SOC_NOPM, 0, 1, 0,
		wcd937x_voip_ec_get, wcd937x_voip_ec_put),
 ...
 #ifdef CONFIG_HW_AUDIO_INFO
 	SOC_ENUM_EXT("SPK_EXT_PA", g_hw_snd_priv_enum[1],
 		simple_pa_switch_get, simple_pa_switch_put),
 	SOC_ENUM_EXT("SIMPLE_PA_Mode", g_hw_snd_priv_enum[2],
 		simple_pa_mode_get, simple_pa_mode_put),
	SOC_ENUM_EXT("HW_HS_SWITCH", g_hw_snd_priv_enum[3],
		hs_switch_get, hs_switch_put),
 #endif
};
