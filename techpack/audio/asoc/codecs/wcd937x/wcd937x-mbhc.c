...
static int wcd937x_spk_pa_status_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	struct wcd937x_mbhc *wcd937x_mbhc = wcd937x_soc_get_mbhc(component);
	...
	ucontrol->value.integer.value[0] = (u32) mbhc->pa_is_power_on;
	dev_info(component->dev, "%s: pa is power on status: %d\n", __func__, mbhc->pa_is_power_on);

	return 0;
}

static int wcd937x_spk_pa_status_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	struct wcd937x_mbhc *wcd937x_mbhc = wcd937x_soc_get_mbhc(component);
	...
	mbhc->pa_is_power_on = ucontrol->value.integer.value[0];
	dev_info(component->dev, "%s: pa is power on status: %d\n", __func__, mbhc->pa_is_power_on);
...
static const struct snd_kcontrol_new spk_pa_status_custom_controls[] = {
	SOC_SINGLE_EXT("SPK_Pa Status", SND_SOC_NOPM, 0, 1, 0,
		       wcd937x_spk_pa_status_get, wcd937x_spk_pa_status_put),
};
		      ...
		      struct fw_info *fw_data)
{
	...
	snd_soc_add_component_controls(component, spk_pa_status_custom_controls,
				ARRAY_SIZE(spk_pa_status_custom_controls));
