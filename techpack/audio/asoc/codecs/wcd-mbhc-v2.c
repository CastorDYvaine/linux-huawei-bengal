...
{
	pr_info("%s mask: 0x%x, status: 0x%x\n", __func__, mask, status);
...
static void wcd_mbhc_set_and_turnoff_hph_padac(struct wcd_mbhc *mbhc)
{
	...
	if (!mbhc->spk_hs_hph_off_control_en) {
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_HPH_PA_EN, 0);
	} else {
		if (!mbhc->pa_is_power_on) {
			WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_HPH_PA_EN, 0);
			pr_debug("%s PA is off\n", __func__);
		} else {
			pr_debug("%s PA is on\n", __func__);
				 ...
				 bool enable)
{
	...
	if (mbhc->disable_elect_insert_irq && irq_type == WCD_MBHC_ELEC_HS_INS) {
		pr_debug("%s: need disable elect_insert_irq\n", __func__);
		return;
	}
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	if (((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) ||
		...
		if (!ana_hs_support_usb_sw() && mbhc->always_enable_micbias_after_insert) {
			pr_info("%s 3.5mm micbias2 AO enable, micbias2 ao\n", __func__);
			micbias_enable = mbhc->micbias_enable;
		...
		if (!ana_hs_support_usb_sw() && mbhc->always_enable_micbias_after_insert) {
			pr_info("%s 3.5mm micbias2 AO disable\n", __func__);
			mbhc->micbias_enable = micbias_enable;
		}
...
static void ana_hs_plug_in_detect(void *priv)
{
	...
	/*
	 * wait 50ms for headset reverse insertion output_mv(hphl/hphr)
	 * slowly climb due to special capacitance
	 */
	msleep(50);

	g_anahs_state = 1;

	if (mbhc->always_enable_micbias_after_insert) {
		pr_info("%s micbias2 AO enable, micbias2 ao\n", __func__);
		micbias_enable = mbhc->micbias_enable;
		mbhc->micbias_enable = true;
	}
		      ...
		      bool impedance_det_en)
{
	...
	mbhc->always_enable_micbias_after_insert = of_property_read_bool(card->dev->of_node, "always_enable_micbias_after_insert");

	if (mbhc->always_enable_micbias_after_insert)
		pr_debug("%s: need always enable micbias after insert\n", __func__);

	mbhc->spk_hs_hph_off_control_en = of_property_read_bool(card->dev->of_node, "qcom,spk-hs-turnoff-hph-bypass");
	if (mbhc->spk_hs_hph_off_control_en)
		pr_debug("%s: hph leave need check the pa status\n", __func__);
	...
	mbhc->disable_elect_insert_irq = of_property_read_bool(card->dev->of_node, "disable_elect_insert_irq");
	if(!mbhc->disable_elect_insert_irq) {
		pr_debug("%s: need enable elect_insert_irq\n", __func__);
		ret = mbhc->mbhc_cb->request_irq(component,
