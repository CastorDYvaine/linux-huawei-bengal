...
#endif
{
	...
	if (enable)
		...
		pr_err ("	yxl  LCD  @dsi_panel_set_pinctrl_state  active \n");
		}
	else
		{
		state = panel->pinctrl.suspend;
		pr_err ("	yxl  LCD  @dsi_panel_set_pinctrl_state  suspend \n");    //GPIO 27  ALWAYS HIGN
		}
...
extern int bkl_backlight_device_set_brightness(struct backlight_device *bl_dev,unsigned long brightness);
...
int dsi_panel_set_priority_recovery_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	int rc = 0;
	struct dsi_backlight_config *bl = &panel->bl_config;
	switch (bl->type) {
#ifdef CONFIG_LCD_KIT_DRIVER
	case DSI_BACKLIGHT_I2C_IC:
		rc = lcd_kit_update_priority_recovery_backlight(bl_lvl);
		break;
	case DSI_BACKLIGHT_DCS_I2C_IC:
		rc = lcd_kit_update_priority_recovery_backlight(bl_lvl);
		break;
...
#ifdef CONFIG_LCD_KIT_DRIVER
	"qcom,mdss-dsi-vint2-exit-before-commands",
	"qcom,mdss-dsi-vint2-exit-after-commands",
#endif
};
...
#ifdef CONFIG_LCD_KIT_DRIVER
	"qcom,mdss-dsi-vint2-exit-before-commands-state",
	"qcom,mdss-dsi-vint2-exit-after-commands-state",
#endif
};
...
static int dsi_panel_parse_esd_config(struct dsi_panel *panel)
{
	...
	esd_config->red_reg_change_enabled = utils->read_bool(utils->data,
		"esd-reg-change-enabled");
	esd_config->tp_esd_support_event = utils->read_bool(utils->data,
		"qcom,tp-esd-support-event");
	...
	esd_config->esd_detect_one_gpio = utils->read_bool(utils->data,
		"qcom,esd-detect-one-gpio");
	esd_config->esd_detect_second_gpio = utils->read_bool(utils->data,
		"qcom,esd-detect-second-gpio");
	if (esd_config->gpio_detect_support) {
		...
		/* 2:detect two gpio */
		if (((dsi_ctrl_count == 2) && !esd_config->esd_detect_one_gpio)
			|| esd_config->esd_detect_second_gpio) {
...
#ifdef CONFIG_LCD_KIT_DRIVER
	if (panel->tp_suspend_before_lcd) {
		rc = lcd_kit_panel_pre_disable(panel);
		return rc;
	}
#endif
