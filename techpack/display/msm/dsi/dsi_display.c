...
// base on dts qcom,mdss-dsi-panel-status-value's index
#define DEMURA_CHECK_REG_INDEX_1 11
#define DEMURA_CHECK_REG_INDEX_2 12
		...
		void *display, u32 bl_lvl)
{
	...
	if (connector->priority_bl_reset_level) {
		rc = dsi_panel_set_priority_recovery_backlight(panel, (u32)connector->priority_bl_reset_level);
		if (rc)
			DSI_ERR("failed to set reset backlight\n");
	}
...
#endif
			if (config->red_reg_change_enabled) {
				if (esd_count_flag == 1 && ((i == DEMURA_CHECK_REG_INDEX_1) || (i == DEMURA_CHECK_REG_INDEX_2))) {
					DSI_INFO("except demura: 0x%x,esd_count_flag=%d i=%d\n", config->status_value[group + i], esd_count_flag, i);
					config->status_value[group + i] = config->return_buf[i];
				}
				DSI_INFO("except value: 0x%x,esd_count_flag=%d i=%d\n", config->status_value[group + i], esd_count_flag, i);
			}
	...
	for (i = 0; i < count; ++i) {
		...
		if (!dsi_display_cmd_is_write(&cmds[i])) { // read succ return read len
			...
			if (rc < 0) {
				...
				if (cmds[i].msg.wait_ms) {
					if (config->red_reg_change_enabled) {
						mdelay(cmds[i].msg.wait_ms);
						DSI_INFO("esd tx cmd transfer mdelay\n");
					} else {
						usleep_range(cmds[i].msg.wait_ms * 1000,
						((cmds[i].msg.wait_ms * 1000) + 10));
					}
				}
...
int dsi_display_gpio_status(struct dsi_display *display, u32 gpio_num, const char *gpio_name, u32 gpio_normal_value)
{
	int value_gpio;
	int rc;
...
int dsi_display_gpio_validate_status(struct dsi_display *display)
{
	...
	rc = dsi_display_gpio_status(display, config->esd_gpio_num0, "gpio0", config->gpio_normal_value0);
	if (rc < 0) {
		DSI_ERR("gpio0 status failed, rc=%d\n", rc);
		return -EPERM;
	}
	/* 2:detect two gpio */
	if (((display->ctrl_count == 2) && !config->esd_detect_one_gpio) || config->esd_detect_second_gpio) {
		rc = dsi_display_gpio_status(display, config->esd_gpio_num1, "gpio1", config->gpio_normal_value1);
		if (rc < 0) {
...
static int dsi_display_status_reg_read(struct dsi_display *display)
{
	...
	struct drm_panel_esd_config *config = &display->panel->esd_config;
	...
	if (config->gpio_detect_support) {
		...
		if (check_times >= GPIO_CHECK_TIMES) {
			...
			if (config->tp_esd_support_event) {
				ts_ops = ts_kit_get_ops();
				if (ts_ops && ts_ops->send_esd_event) {
					rc = ts_ops->send_esd_event(TP_REPORT_ESD_STATUS);
					if (rc)
						DSI_ERR("send_esd_event failed\n");
				}
			}
		...
		int power_mode, void *disp)
{
	...
	case SDE_MODE_DPMS_LP1:
		lcd_kit_ts_aod_state_notify(display->panel);
