...
static bool tcp_should_recover(struct sock *sk)
{
#ifdef CONFIG_HW_NETWORK_QOE
	struct tcp_sock *tp = tcp_sk(sk);
	if (tp->should_recover) {
		pr_warn("tcp_should_recover: uid:%d isTcpRecoverScene:%d\n", sk->sk_uid.val, tp->should_recover);
		return true;
	}
#endif
	return false;
...
void tcp_rcv_established(struct sock *sk, struct sk_buff *skb)
{
	...
	tp->rx_opt.saw_tstamp = 0;
#ifdef CONFIG_HW_BOOSTER
	hook_dl_stub(sk, skb, tp->tcp_header_len);
#endif
