...
int smc_set_init(struct enable_msg *msg)
{
	...
	if ((msg->reg_num < 0) || (msg->reg_num > RSMC_REG_NUM)) {
		hwlog_err("%s: invalid reg num", __func__);
		return -EINVAL;
	}
	if ((msg->rf_num < 0) || (msg->rf_num > RSMC_RF_NUM)) {
		hwlog_err("%s: invalid rf num", __func__);
		return -EINVAL;
	}
