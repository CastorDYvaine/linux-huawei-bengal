...
typedef int (*lld_tz_query_fn)(struct request_queue *q, u32 type, u8 *buf, u32 buf_len);
...
#ifdef CONFIG_IOSCHED_FMQ
	struct list_head	rl_head[2];
	struct bio		*cur_bio;
	int (*get_inflight_rq_fn)(struct request_queue *q, unsigned int type);
#endif
