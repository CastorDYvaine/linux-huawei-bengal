...
retry_cpuset:
	...
	*/
	...
	if (blkcg_type > BLK_THROTL_KBG && can_direct_reclaim && !(current->flags & PF_MEMALLOC) &&
		!(fatal_signal_pending(current) && !(gfp_mask & __GFP_NOFAIL) && (gfp_mask & __GFP_FS))) {
...
void set_default_fault_around_bytes(void)
{
	...
	if (total_present_mbytes <= MEMSIZE_3G_IN_MB)
		set_fault_around_bytes(1 * PAGE_SIZE);
}
...
static void __setup_per_zone_wmarks(void)
{
	...
	set_default_fault_around_bytes();
