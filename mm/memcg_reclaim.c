...
bool shrink_nodes(pg_data_t *pgdat, struct scan_control *sc)
{
	...
	do {
		...
		/* only kswapd will shrink slab objs when RAM Turbo enable */
		...
		struct mem_cgroup *memcg)
{
	...
	nr_reclaimed += lru_active_anon_size;
...
static int zswapd(void *p)
{
...
#ifdef CONFIG_RAMTURBO
	static DEFINE_RATELIMIT_STATE(zswapd_report, 1*HZ, 1);
#endif
	...
	#ifdef CONFIG_RAMTURBO
		if (ramturbo_enable() && !__ratelimit(&zswapd_report))
			continue;
	#endif
