...
static int kswapd_high_load_swappiness = 60;
		...
		char *buf, size_t nbytes, loff_t off)
{
	...
	for_each_online_node(nid) {
		iter_start = jiffies;
		pgdat = NODE_DATA(nid);
		single_reclaimed_pages = all_active_reclaim_by_memcg(pgdat, memcg);
		iter_end = jiffies;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) && defined(CONFIG_ION)
	seq_printf(m, "IonTotalUsed:%lu\n",
		ion_get_total_heap_bytes() / SZ_1K);
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	seq_printf(m, "GpuTotalUsed:%lu\n",
		gpu_get_total_used() * PAGE_SIZE / SZ_1K);
#endif
	seq_printf(m, "PageTables:%lu\n",
		global_zone_page_state(NR_PAGETABLE) * PAGE_SIZE / SZ_1K);
	seq_printf(m, "SReclaimable:%lu\n",
		global_node_page_state(NR_SLAB_RECLAIMABLE) * PAGE_SIZE / SZ_1K);
	seq_printf(m, "SUnreclaim:%lu\n",
		global_node_page_state(NR_SLAB_UNRECLAIMABLE) * PAGE_SIZE / SZ_1K);
	seq_printf(m, "KernelStack:%lu\n",
		global_zone_page_state(NR_KERNEL_STACK_KB));
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) && defined(CONFIG_CMA)
...
static int kswapd_high_load_threshold_show(struct seq_file *m, void *v)
{
	seq_printf(m, "kswapd_high_load_threshold: %lu\n",
			kswapd_high_load_threshold);
#ifdef CONFIG_KSWAPD_DEBUG
	seq_printf(m, "kswapd_called_cnt: %lu\n",
			atomic64_read(&kswapd_called_cnt));
	seq_printf(m, "kswapd_in_dr_cnt: %lu\n",
			atomic64_read(&kswapd_in_dr_cnt));
	seq_printf(m, "kswapd_high_load_cnt: %lu\n",
...
#ifdef CONFIG_HYPERHOLD_DEBUG
		.seq_show = kswapd_high_load_threshold_show,
#endif
	},
#endif
#ifdef CONFIG_HYPERHOLD_DEBUG
