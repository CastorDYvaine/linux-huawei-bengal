...
int pa_gpio;
static bool support_one_id_pin;
static bool need_time_control;
static DEFINE_SPINLOCK(fs15xx_lock);
static DEFINE_SPINLOCK(sia_lock);
#define SIA_PULSE_GAP_TIME_US 10
#define SIA_DOWN_TIME_US_MIN 5000
#define SIA_DOWN_TIME_US_MAX 6000
#define SIA_UP_TIME_US_MIN 1000
#define SIA_UP_TIME_US_MAX 2000
...
int hw_simple_pa_power_set(int gpio, int value)
{
	...
	if (value == GPIO_PULL_UP) {
		...
		} else if (need_time_control && priv->pa_type == SIMPLE_PA_SIA) {
			pr_info("%s set spin_lock\n", __func__);
			gpio_set_value(gpio, GPIO_PULL_DOWN);
			usleep_range(SIA_DOWN_TIME_US_MIN, SIA_DOWN_TIME_US_MAX);
			spin_lock_irqsave(&sia_lock, gpio_flag);
			for (i = 0; i < priv->pa_mode; i++) {
				gpio_set_value(gpio, GPIO_PULL_DOWN);
				udelay(SIA_PULSE_GAP_TIME_US);
				gpio_set_value(gpio, GPIO_PULL_UP);
				udelay(SIA_PULSE_GAP_TIME_US);
			}
			spin_unlock_irqrestore(&sia_lock, gpio_flag);
			usleep_range(SIA_UP_TIME_US_MIN, SIA_UP_TIME_US_MAX);
		} else {
			gpio_set_value(gpio, GPIO_PULL_DOWN);
			msleep(1);
			for (i = 0; i < priv->pa_mode; i++) {
		...
		gpio_set_value(gpio, GPIO_PULL_DOWN);
		if (need_time_control && priv->pa_type == SIMPLE_PA_SIA)
			usleep_range(SIA_DOWN_TIME_US_MIN, SIA_DOWN_TIME_US_MAX);
	}
	return 0;
}
			   ...
			   const char *gpio_name, int index, int *gpio_status)
{
	...
	if (support_one_id_pin && index == 0) {
		gpio_direction_output(gpio_id[0], GPIO_PULL_DOWN);
		gpio_status[index] = 0;
		pr_err("%s: id%d set default value\n", __func__, index);
		goto default_value;
	}
...
static int audio_info_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(pdev->dev.of_node, "support_one_id_pin")) {
		support_one_id_pin = true;
	} else {
		support_one_id_pin = false;
	}

	if (of_property_read_bool(pdev->dev.of_node, "need_time_control"))
		need_time_control = true;
	else
