...
static DEFINE_MUTEX(dubai_set_gpu_lock);
...
long dubai_ioctl_gpu(unsigned int cmd, void __user *argp)
{
	...
	switch (cmd) {
	case IOC_GPU_ENABLE:
		mutex_lock(&dubai_set_gpu_lock);
		rc = dubai_set_gpu_enable(argp);
		mutex_unlock(&dubai_set_gpu_lock);
		break;
...
static int dubai_get_gpu_info(void __user *argp)
{
	...
	memset_s(&delta_stats, sizeof(struct dubai_gpu_stats), 0, sizeof(struct dubai_gpu_stats));
