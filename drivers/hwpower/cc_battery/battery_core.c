...
static const char * const g_bat_core_prop_table[] = {
	[BAT_CORE_PROP_CYCLE] = "bat_cycle",
};

static int bat_core_get_prop_type(const char *str)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_bat_core_prop_table); i++) {
		if (strcmp(str, g_bat_core_prop_table[i]) == 0)
			return i;
	}
...
static int bat_core_check_prop_type(int type)
{
	if ((type >= BAT_CORE_PROP_BEGIN) && (type < BAT_CORE_PROP_END))
		return 0;
...
static struct bat_core_ops *bat_core_get_ops(int type)
{
	if (bat_core_check_prop_type(type))
		return NULL;

	if (!g_bat_core_dev || !g_bat_core_dev->p_ops[type]) {
		hwlog_err("g_bat_core_dev or p_ops is null\n");
		return NULL;
	}

	return g_bat_core_dev->p_ops[type];
}

int bat_core_ops_register(struct bat_core_ops *ops)
{
	int type;

	if (!g_bat_core_dev || !ops || !ops->type_name) {
		hwlog_err("g_bat_core_dev or ops or type_name is null\n");
		return -EPERM;
	}

	type = bat_core_get_prop_type(ops->type_name);
	if (type < 0) {
		hwlog_err("%s ops register fail\n", ops->type_name);
		return -EPERM;
	}

	g_bat_core_dev->p_ops[type] = ops;
	g_bat_core_dev->total_ops++;

	hwlog_info("total_ops=%d type=%d:%s ops register ok\n",
		g_bat_core_dev->total_ops, type, ops->type_name);
	return 0;
}

int bat_core_ops_unregister(struct bat_core_ops *ops)
{
	int type;
	...
	type = bat_core_get_prop_type(ops->type_name);
	if (type < 0) {
		hwlog_err("%s ops unregister fail\n", ops->type_name);
		return -EPERM;
	}

	if (g_bat_core_dev->p_ops[type] != ops) {
		hwlog_err("ops not same, cannot unregister\n");
		return -EPERM;
	}

	g_bat_core_dev->p_ops[type] = NULL;
	g_bat_core_dev->total_ops--;

	hwlog_info("total_ops=%d type=%d:%s ops unregister ok\n",
		g_bat_core_dev->total_ops, type, ops->type_name);
	return 0;
}

static void bat_core_set_ops_cycle(int cycle_count)
{
	struct bat_core_ops *l_ops = bat_core_get_ops(BAT_CORE_PROP_CYCLE);
	if (l_ops && l_ops->set_cycle_count)
		l_ops->set_cycle_count(cycle_count);
}
...
static int bat_core_get_cycle_count_raw(void)
{
...
static int bat_core_get_cycle_count(void)
{
	...
	if (l_ops && l_ops->get_cycle_count)
		cycle = l_ops->get_cycle_count();

	if (cycle >= 0)
		return cycle;
	else
		return bat_core_get_cycle_count_raw();
}
...
static void bat_core_update_data(struct bat_core_device *di)
{
	...
	if (!t_data.exist) {
		...
		bat_core_set_ops_cycle(t_data.cycle_count);
	}
