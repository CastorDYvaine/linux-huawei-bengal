...
static int bat_cycle_read_from_nv(void)
{
	struct bat_cycle_device *di = g_bat_cycle_device;
	int ret;
	...
	ret = power_nv_read(POWER_NV_BATCYC, &(di->data), sizeof(di->data));
	if (ret)
		return ret;

	hwlog_info("data read from nv:\n");
...
static int bat_cycle_get_coul_cycle_count(void)
{
	...
	return (cycle >= 0) ? cycle * BAT_CYC_UNIT : cycle;
}
...
static ssize_t bcyc_ft_remind_show(struct device *dev, struct device_attribute *attr, char *buff)
{
	...
	if ((di->ft_remind_mode == BAT_CYC_FT_REMIND_MODE_BATT_REMOVED_NOCT) && di->batt_has_sn)
		flag = 0;
	if (di->ft_remind_mode == BAT_CYC_FT_REMIND_MODE_ALWAYS)
		flag = 1;
 ...
 */
static ssize_t bcyc_nv_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct bat_cycle_device *di = g_bat_cycle_device;
	int index = 0;
	...
	if (ret != 9) {
		hwlog_err("[%s] unable to parse input:%s, ret=%d\n", __func__, buf, ret);
		return -EINVAL;
	}
	di->data.index = index;
...
static ssize_t bcyc_nv_show(struct device *dev, struct device_attribute *attr, char *buff)
{
	struct bat_cycle_device *di = g_bat_cycle_device;
	int len = 0;
	int i;
	...
	len += snprintf_s(buff + len, PAGE_SIZE - len, PAGE_SIZE - len - 1,
		"index:%d,ver:%d,last_cycle:%d\n", di->data.index, di->data.ver, di->data.last_cycle);
	for (i = 0; i < BAT_CYC_NV_REC_CNT; i++)
		len += snprintf_s(buff + len, PAGE_SIZE - len, PAGE_SIZE - len - 1,
			"[%d]SN:%s,cyc:%d\n", i, di->data.records[i].sn, di->data.records[i].cycle);

	return len;
}
static ssize_t bcyc_remind_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct bat_cycle_device *di = g_bat_cycle_device;
	int ret;
	...
	ret = sscanf_s(buf, "%d", &(di->ft_remind_mode));
	if (ret != 1) {
		hwlog_err("[%s] unable to parse input:%s, ret=%d\n", __func__, buf, ret);
		return -EINVAL;
	}
...
static ssize_t bcyc_remind_mode_show(struct device *dev, struct device_attribute *attr, char *buff)
{
	struct bat_cycle_device *di = g_bat_cycle_device;
	if (!di) {
		hwlog_err("[%s] cycle drv is null\n", __func__);
	...
	return snprintf_s(buff, BAT_CYC_SYSFS_BUFF_SIZE, BAT_CYC_SYSFS_BUFF_SIZE, "%d", di->ft_remind_mode);
}

#endif /* CONFIG_HUAWEI_POWER_DEBUG */

static const DEVICE_ATTR_RO(bcyc_battery_removed);
static const DEVICE_ATTR_RO(bcyc_ft_remind);
static const DEVICE_ATTR_RW(bcyc_ft_operate);
#ifdef CONFIG_HUAWEI_POWER_DEBUG
static const DEVICE_ATTR_RW(bcyc_nv);
static const DEVICE_ATTR_RW(bcyc_remind_mode);
#endif /* CONFIG_HUAWEI_POWER_DEBUG */
...
#ifdef CONFIG_HUAWEI_POWER_DEBUG
	&dev_attr_bcyc_nv.attr,
	&dev_attr_bcyc_remind_mode.attr,
#endif /* CONFIG_HUAWEI_POWER_DEBUG */
	NULL,
};
...
static int bat_cycle_get_battery_sn(char *buff)
{
	...
	if (ret) {
		hwlog_err("get battery identifier fail, ret=%d\n", ret);
		return ret;
	}

	if (di->battery_type == BAT_CYC_BATTERY_TYPE_2_BATT) {
		ret = get_battery_group_sn(res, BAT_CYC_SN_BUFF_LEN);
		if (ret) {
			hwlog_err("get battery group sn fail\n");
			return ret;
		}
	}
...
static int bat_cycle_init_sn()
{
	...
	ret = bat_cycle_get_battery_sn(di->batt_sn);
	if (ret == 0)
		return 0;

	if (di->batt_sn_retry_mode == BAT_CYC_SN_RETRY_MODE_NEVER)
		return strncpy_s(di->batt_sn, BAT_CYC_SN_STR_LEN, BAT_CYC_SN_NO_CT_STR, BAT_CYC_SN_LEN);
	if ((di->batt_sn_retry_mode == BAT_CYC_SN_RETYR_MODE_NORMAL) &&
		((ret == -ENOTSUPP) || (ret == -EUNATCH))) {
		hwlog_info("no SN on this battery, ret=%d\n", ret);
		return strncpy_s(di->batt_sn, BAT_CYC_SN_STR_LEN, BAT_CYC_SN_NO_CT_STR, BAT_CYC_SN_LEN);
	}
	hwlog_err("get battery sn fail, %d\n", di->init_retry_times);
	if (di->init_retry_times > BAT_CYC_INIT_SN_MAX_RETRY)
		return strncpy_s(di->batt_sn, BAT_CYC_SN_STR_LEN, BAT_CYC_SN_NO_CT_STR, BAT_CYC_SN_LEN);
	return -1;
}
...
static void bat_cycle_init_work(struct work_struct *work)
{
	...
	if (bat_cycle_init_sn())
		goto delay_retry;

	di->batt_has_sn = (strcmp(di->batt_sn, BAT_CYC_SN_NO_CT_STR) == 0) ? 0 : 1;
	hwlog_info("battery SN:%s, has_sn:%d\n", di->batt_sn, di->batt_has_sn);
...
delay_retry:
	...
	if (di->init_retry_times > BAT_CYC_INIT_MAX_RETRY) {
		hwlog_err("bat cycle init failed after %d retries\n", BAT_CYC_INIT_MAX_RETRY);
...
static int bat_cycle_dts_parse(struct device_node *np, struct bat_cycle_device *di)
{
	...
	ret += power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "batt_sn_retry_mode", &(di->batt_sn_retry_mode), 1);
	return ret;
}
