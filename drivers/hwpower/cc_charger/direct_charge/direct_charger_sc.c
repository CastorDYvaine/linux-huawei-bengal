...
static int sc_get_charging_info(char *buf, unsigned int len)
{
	int ret;
	int i = 0;
	char temp_buf[DC_SC_CUR_LEN] = {0};
...
static void sc_fault_work(struct work_struct *work)
{
	...
	case POWER_NE_DC_FAULT_CC_SHORT:
		...
		break;
	case POWER_NE_DC_FAULT_I2C_ERR:
		hwlog_err("fake i2c read/write err happend\n");
		snprintf(buf, sizeof(buf),
			"device id: 0x%x. fake i2c read/write error!\n", data->event1);
		power_dsm_report_dmd(POWER_DSM_DIRECT_CHARGE_SC,
			ERROR_CHARGE_I2C_RW, buf);
		break;
	default:
...
static struct power_sysfs_attr_info sc_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_ro(sc, 0444, DC_SYSFS_ADAPTER_ANTIFAKE_STATE, adaptor_antifake_state),
	power_sysfs_attr_ro(sc, 0444, DC_SYSFS_CABLE_TYPE, cable_type),
	...
	power_sysfs_attr_ro(sc, 0444, DC_SYSFS_CHARGING_INFO, charging_info),
	...
	struct device_attribute *attr, char *buf)
{
	...
	case DC_SYSFS_CHARGING_INFO:
		len = sc_get_charging_info(buf, PAGE_SIZE);
		break;
