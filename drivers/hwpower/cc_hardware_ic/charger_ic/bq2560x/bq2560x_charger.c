...
static void sgm41512_get_bc12_result(struct bq2560x_device_info *di)
{
	...
	if (vbus_stat == REG08_VBUS_TYPE_NONE) {
		hwlog_err("%s: vbus_stat is 0\n", __func__);
		(void)power_msleep(DT_MSLEEP_100MS, 0, NULL);
		ret = bq2560x_read_byte(BQ2560X_REG_SS, &reg);
		if (ret < 0)
			hwlog_err("%s: reg read fail 2\n", __func__);
...
static void bq2560x_vbus_good_check(void)
{
	u8 reg_0x08 = 0;
	u8 reg_0x0a = 0;
	int i;
...
static void bq2560x_bc12_done_check(struct bq2560x_device_info *di)
{
	int ret;
	u8 reg = 0;
	int bc12_done;
	...
	/* timeout:200ms*10 = 2000ms, if bc12 is not done, wait 200ms and read again */
	for (i = 0; i < 10; i++) {
		if (di->vendor_id == ETA6963_VENDOR_ID) {
			ret = bq2560x_read_byte(BQ2560X_REG_SS, &reg);
			hwlog_info("reg[%d]=0x%x\n", BQ2560X_REG_SS, reg);
			if (ret < 0)
				hwlog_err("bq2560x_read_byte fail\n");
			bc12_done = (reg & BQ2560X_REG_SS_VBUS_STAT_MASK) >>
				BQ2560X_REG_SS_VBUS_STAT_SHIFT;
			if (bc12_done != 0)
				return;
		} else {
		...
		power_msleep(DT_MSLEEP_200MS, 0, NULL); /* wait bc1.2 done */
	}
	hwlog_err("%s: input detection never done\n", __func__);
}
...
static int sgm41512_chg_type_det(struct bq2560x_device_info *di, bool en)
{
	...
	if (!en) {
		di->chg_type = CHARGER_REMOVED;
		bq2560x_read_byte(BQ2560X_REG_SS, &reg);
		hwlog_info("charger_removed reg[%d]=0x%x, chg_type=%d\n",
			BQ2560X_REG_SS, reg, di->chg_type);
		sgm41512_inform_psy_changed(di);
		return 0;
	}

	bq2560x_vbus_good_check();
...
static int bq2560x_config_prechg_current(int value)
{
	int val;

	if (value < IPRECHRG_MIN_60)
		value = IPRECHRG_MIN_60;
	else if (value > IPRECHRG_MAX_780)
		value = IPRECHRG_MAX_780;

	val = (value - REG03_IPRECHG_BASE) / REG03_IPRECHG_LSB;

	hwlog_info("iprechg_cur [%x]=0x%x, value = %d\n", BQ2560X_REG_PCTCC, val, value);
	return bq2560x_write_mask(BQ2560X_REG_PCTCC,
		BQ2560X_REG_PCTCC_IPRECHG_MASK,
		BQ2560X_REG_PCTCC_IPRECHG_SHIFT,
		val);
}
...
static int bq2560x_reused_set_iterm(int value)
{
	...
	hwlog_info("reused_set_iterm [%x]=0x%x, value = %d\n", BQ2560X_REG_PCTCC, val, value);
...
static int bq2560x_5v_chip_init(struct bq2560x_device_info *di)
{
	...
	/* default init iprechg = 300ma,iterm current = 180ma */
	ret = bq2560x_config_prechg_current(di->iprechg_cur);
	if (ret < 0)
		hwlog_err("init pre_chg_cur fail\n");

	ret = bq2560x_reused_set_iterm(di->ieoc);
	if (ret < 0)
		hwlog_err("init iterm fail\n");
...
static int bq2560x_set_terminal_current(int value)
{
	...
	if ((ichg > LIMIT_CURR) && !di->no_need_change_iterm) {
...
static int bq2560x_get_charge_state(unsigned int *state)
{
	...
	if (((reg & BQ2560X_REG_F_FAULT_CHRG_MASK) >>
		BQ2560X_REG_F_FAULT_CHRG_SHIFT) ==
		REG09_FAULT_CHRG_INPUT)
...
static int bq2560x_get_vbus_mv(int *vbus_mv)
{
	int ret;
	u32 state = 0;
...
static int bq2560x_switch_chg_type_det(bool en)
{
	...
	if (g_bq2560x_dev->chg_det_enable)
		sgm41512_chg_type_det(g_bq2560x_dev, en);
...
static int bq2560x_charge_enable(int enable, void *dev_data)
{
	...
	if (enable == 0) {
		bq2560x_set_ovpthreshold(di->dischg_vac_ovp);
		return charge_set_hiz_enable(HIZ_MODE_ENABLE);
	}

	bq2560x_set_ovpthreshold(di->chg_en_vac_ovp);
	...
	const struct i2c_device_id *id)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"chg_en_vac_ovp", &(di->chg_en_vac_ovp), REG06_OVP_14P3V);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"dischg_vac_ovp", &(di->dischg_vac_ovp), REG06_OVP_10P5V);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"iprechg_cur", &(di->iprechg_cur), BQ2560X_DEFAULT_IPRECHG_CURRENT);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
