...
static int rt9471_parse_dt(struct rt9471_chip *chip)
{
	...
	chip->write_back_vacovp = of_property_read_bool(np, "write_back_vacovp");
...
static void rt9471_shutdown(struct i2c_client *client)
{
	...
	if (chip->write_back_vacovp) {
		...
		ret = rt9471_i2c_write_byte(chip, RT9471_REG_VBUS, regval);
	} else {
		rt9471_reset_register(chip);
	}
}
