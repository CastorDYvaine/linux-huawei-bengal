...
#define HIMAX_STYLUS_CONNECT_REG 0x10007F34
#define HIMAX_STYLUS_CONNECT_VAL 0xA55AA55A
#define HIMAX_STYLUS_DISCONNECT_VAL 0x00000000
#define HIMAX_STYLUS3_CON 2
#define HIMAX_BT_SEND_CMD_RETRY 3
#define HIMAX_SEND_BT_CONNECT_STATUS_AFTER_RESET_DELAY 2
...
static int touch_driver_send_stylus_cmd(struct thp_device *tdev,
	bool stylus3_uplink_switch)
{
	int ret;
	int i = 0;
	uint8_t tmp_addr[HIMAX_NORMAL_ADDR_LEN] = { 0 };
	uint8_t tmp_data[HIMAX_NORMAL_DATA_LEN] = { 0 };
	uint8_t read_buf[HIMAX_NORMAL_ADDR_LEN] = { 0 };

	himax_assign_data(HIMAX_STYLUS_CONNECT_REG, tmp_addr);
	if (stylus3_uplink_switch)
		himax_assign_data(HIMAX_STYLUS_CONNECT_VAL, tmp_data);
	else
		himax_assign_data(HIMAX_STYLUS_DISCONNECT_VAL, tmp_data);

	while (i < HIMAX_BT_SEND_CMD_RETRY) {
		touch_driver_register_write(tdev, tmp_addr,
			HIMAX_NORMAL_ADDR_LEN, tmp_data);

		ret = touch_driver_register_read(tdev, tmp_addr,
			HIMAX_NORMAL_ADDR_LEN, read_buf);
		if (ret)
			thp_log_err("%s: read stylus reg fail\n", __func__);

		if (!memcmp(tmp_data, read_buf, HIMAX_NORMAL_ADDR_LEN)) {
			thp_log_info("%s: check stylus cmd succ\n", __func__);
			return 0;
		}
		i++;
	}

	thp_log_info("%s: check stylus cmd fail\n", __func__);
	return -EIO;
}

static int touch_driver_hx_get_power_status(struct thp_device *tdev)
{
...
static int touch_driver_hx_bt_handler(struct thp_device *tdev,
	bool delay_enable)
{
	int ret;
	...
	ret = touch_driver_hx_get_power_status(tdev);
	if (ret < 0)
		return 0;
	...
	thp_log_info("%s: get stylus3 status: %u, current_stylus3_work_mode:%u\n",
		__func__, stylus3_status, current_stylus3_work_mode);
	current_stylus3_status = ((stylus3_status & HIMAX_STYLUS3_CON) > 0) &&
		(current_stylus3_work_mode == 0);
	if (delay_enable)
		mdelay(HIMAX_SEND_BT_CONNECT_STATUS_AFTER_RESET_DELAY);

	ret = touch_driver_send_stylus_cmd(tdev, current_stylus3_status);
	if (ret < 0) {
		thp_log_info("%s: send stylus fail\n", __func__);
		return -EIO;
	}
	...
	const u8 *data, unsigned int *gesture_wakeup_value)
{
	...
	if (!strncmp(cd->vendor_name,
		...
		VENDOR_NAME_TCL, THP_PROJECTID_VENDOR_NAME_LEN)) {
		tx_num = HX_TCL_TX_NUM;
		rx_num = HX_TCL_RX_NUM;
	} else {
...
static int thp_of_property_read_u32(struct device_node *hx83112_node,
	char *propname, int *out_val)
{
	int ret;
	unsigned int value = 0;
	unsigned int invald_arg_flag = 0;

	invald_arg_flag = (!hx83112_node) || (!propname) || (!out_val);
	if (invald_arg_flag) {
		thp_log_err("%s: node name out_val null\n", __func__);
		return -EINVAL;
	}

	ret = of_property_read_u32(hx83112_node, propname, out_val);
	if (ret) {
		*out_val = value;
		thp_log_err("%s:%s not configed\n", __func__, propname);
	} else {
		thp_log_info("%s:%s configed value = %d",
			__func__, propname, *out_val);
	}

	return ret;
}

static int thp_parse_ic_feature_config(struct thp_device *tdev,
	struct device_node *hx83112_node)
{
	struct himax_thp_private_data *himax_p = NULL;
	unsigned int invald_arg_flag = 0;

	invald_arg_flag = (!hx83112_node) || (!tdev);
	if (invald_arg_flag) {
		thp_log_err("%s: tdev or device_node null\n", __func__);
		return -EINVAL;
	}
	himax_p = tdev->private_data;
	...
	thp_of_property_read_u32(hx83112_node,
		"get_frame_optimized_method",
		&himax_p->hx_get_frame_optimized_flag);

	thp_of_property_read_u32(hx83112_node,
		"himax_ic_83102_flag", &himax_p->himax_ic_83102_flag);

	thp_of_property_read_u32(hx83112_node,
		"himax_ic_83102_no_flash", &himax_p->himax_ic_83102_no_flash);

	thp_of_property_read_u32(hx83112_node,
		"himax_ic_hispeed_support", &himax_p->himax_ic_hispeed_support);

	thp_of_property_read_u32(hx83112_node,
		"himax_gesture_need_lcd_rst",
		&himax_p->himax_gesture_need_lcd_rst);
...
int touch_driver_init(struct thp_device *tdev)
{
	...
	rc = thp_parse_ic_feature_config(tdev, hx83112_node);
	if (rc)
		thp_log_err("%s: ic_feature parse fail\n", __func__);
...
int touch_driver_83102_chip_detect(struct thp_device *tdev)
{
	...
	himax_p = tdev->private_data;
	if (!himax_p->himax_ic_83102_no_flash) {
	...
	ret = touch_driver_hx_bt_handler(tdev, false);
	if (ret)
	...
	char *buf, unsigned int len)
{
	...
	if ((!tdev) || (!buf) || (!len)) {
		thp_log_err("%s: tdev or buf or len is null\n", __func__);
		return -EINVAL;
	}
	himax_p = tdev->private_data;
	...
	if (!strncmp(temp_buf,
		...
		VENDOR_ID_TCL, THP_PROJECTID_VENDOR_NAME_LEN)) {
		cd->vendor_name = VENDOR_NAME_TCL;
	} else {
		thp_log_err("%s: not support this vendor_id\n", __func__);
		return -EINVAL;
	}
...
static struct thp_device_ops hx83112_dev_ops = {
	...
	.bt_handler = touch_driver_hx_bt_handler,
};
