...
static uint8_t stylus_enable_cmd[] = {
	TOUCH_NORM_WR_CMD, 0x5e, 0x22, 0x01, 0x0, 0x33, 0x36,
	0x01, 0x05, 0x80
};

static uint8_t stylus_disable_cmd[] = {
	TOUCH_NORM_WR_CMD, 0x5e, 0x22, 0x01, 0x0, 0x33, 0x36,
	0x0, 0x0, 0x0
};

/* Workmode cmd */
static uint8_t get_workmode_cmd[] = {
	...
	void *tbuf, size_t tlen)
{
	...
	rc = touch_driver_spi_transfer(tdev, tbuf, g_cmd_rbuf, tlen);
	if (rc) {
		thp_log_err("%s:cspi_transfer send failed: rc=%d\n", __func__, rc);
		return rc;
	}
	memset(g_cmd_rbuf, 0, CMD_BUF_SIZE);
	rc = touch_driver_spi_transfer(tdev, tbuf, g_cmd_rbuf,
	...
	struct thp_core_data *cd)
{
	...
	rc = of_property_read_u32(thp_node, "touch_frame_size",
		&g_touch_frame_size);
	if (!rc)
		thp_log_info("%s: touch_frame_size %u\n",
			__func__, g_touch_frame_size);
...
static int touch_driver_check_stylus_cmd(struct thp_device *tdev, int cmd_size)
{
	...
	rc = touch_driver_spi_transfer(tdev, NULL, tdev->rx_buff,
		TOUCH_ENTER_STYLUS_CHECK_LEN);
	if (rc)
		thp_log_err("%s: read failed: rc=%d", __func__, rc);

	check_cmd_flag = (tdev->rx_buff[0] == ENTER_STYLUS_ERR_CHECK) &&
		(tdev->rx_buff[1] == ENTER_STYLUS_CMD1_CHECK) &&
		(tdev->rx_buff[2] == ENTER_STYLUS_CMD2_CHECK);
	if (check_cmd_flag) {
	...
	bool stylus3_uplink_switch)
{
	...
	if (stylus3_uplink_switch) {
		cmd_size = sizeof(stylus_enable_cmd);
		rc = memcpy_s(tdev->tx_buff, cmd_size,
			stylus_enable_cmd, cmd_size);
	} else {
		cmd_size = sizeof(stylus_disable_cmd);
		rc = memcpy_s(tdev->tx_buff, cmd_size,
			stylus_disable_cmd, cmd_size);
	}
	if (rc != EOK) {
	...
	while (i < TOUCH_SEND_CMD_RETRIES) {
	...
	bool delay_enable)
{
	...
	rc = touch_driver_send_stylus_cmd(tdev, current_stylus3_status);
	if (rc < 0)
...
static int touch_driver_detect(struct thp_device *tdev)
{
	...
	for (retry = 0; retry < DETECT_RETRY_TIME; retry++) {
		...
		if (hwid != ICNL9922C_HWID && hwid != ICNL9951R_HWID) {
			...
			rc = touch_driver_bt_handler(tdev, false);
			if (rc)
				thp_log_err("%s: send stylus3 status fail\n", __func__);
...
{
	unsigned int copy_len = len < g_touch_frame_size ? len : g_touch_frame_size;
	int rc;

	if (g_touch_frame_size > PAGE_SIZE) {
		thp_log_err("%s: g_touch_frame_size Exceeded the limitc %u\n",
			__func__, g_touch_frame_size);
		return -EINVAL;
	}
	rc = thp_bus_lock();
	...
	memset(tdev->tx_buff, 0, g_touch_frame_size);
	memset(tdev->rx_buff, 0, g_touch_frame_size);
	memcpy(tdev->tx_buff, get_frame_cmd, sizeof(get_frame_cmd));
	/* tx_buff and rx_buff is for irq use only */
	rc = touch_driver_spi_transfer(tdev, tdev->tx_buff,
		tdev->rx_buff, g_touch_frame_size);
	if (rc) {
	...
	if (len > g_touch_frame_size)
		memset(buf + g_touch_frame_size, 0, len - g_touch_frame_size);
...
static void touch_read_workmode(struct thp_device *tdev)
{
	...
	thp_bus_lock();
	rc = touch_dual_protocols_spi_transfer(tdev,
		get_workmode_cmd, sizeof(get_workmode_cmd));
	if (rc)
