	...
	unsigned int have_time_data)
{
	...
	if (!have_time_data)
		thp_coordinate_report(&data, have_time_data);
	else
...
static void thp_send_event_to_ultrasonic(unsigned int value)
{
	struct thp_core_data *cd = thp_get_core_data();
...
static long thp_mt_wrapper_ioctl_set_events(unsigned long arg)
{
	...
	cd->event_flag = true;
	cd->event = val;
	thp_send_event_to_ultrasonic(val);
	if (cd->event_flag) {
		cd->thp_event_waitq_flag = WAITQ_WAKEUP;
...
static int thp_mt_ioctl_report_keyevent(unsigned long arg)
{
	...
	if (cd->suspend_info.aod_and_proximity_support &&
		(cd->aod_notify_tp_power_status == AOD_NOTIFY_TP_SUSPEND)) {
		thp_log_err("%s: in aod mode, not support prox event\n", __func__);
		return 0;
	}
...
static int thp_ioctl_set_tp_broken_info(unsigned long arg)
{
	void __user *argp = (void __user *)(uintptr_t)arg;
	...
	unsigned long arg)
{
	...
	case INPUT_MT_WRAPPER_IOCTL_CMD_SET_COORDINATES:
		ret = thp_mt_wrapper_ioctl_set_coordinate(arg, have_time_data);
		break;
	...
	case INPUT_MT_IOCTL_CMD_SET_AOD_STATUS:
		...
		break;
	case INPUT_MT_IOCTL_CMD_SET_STYLUS3_WORK_MODE:
		ret = thp_wrapper_ioctl_set_stylus3_work_mode(arg);
		break;
	default:
	...
	struct thp_mt_wrapper_data *mt_wrapper)
{
	...
	thp_input_dev_set_params(input_dev, mt_wrapper);
...
int thp_mt_wrapper_init(void)
{
	...
	rc = thp_input_dev_register(input_dev, mt_wrapper);
	if (rc)
