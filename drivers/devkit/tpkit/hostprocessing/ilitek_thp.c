...
static void thp_parse_aod_config(struct device_node *thp_node,
	struct thp_core_data *cd)
{
	if (of_property_read_u32(thp_node, "aod_and_proximity_support",
		&cd->suspend_info.aod_and_proximity_support))
		cd->suspend_info.aod_and_proximity_support = 0;

	thp_log_info("%s: aod_and_proximity_support = %u\n",
		__func__, cd->suspend_info.aod_and_proximity_support);

	if (of_property_read_u32(thp_node, "aod_support_on_tddi", &cd->aod_support_on_tddi))
	...
	struct thp_core_data *cd)
{
	...
	thp_parse_aod_config(thp_node, cd);
...
static int touch_driver_suspend(struct thp_device *tdev)
{
	...
	gesture_status = ((cd->support_gesture_mode) &&
		(cd->easy_wakeup_info.sleep_mode == TS_GESTURE_MODE) && (!cd->support_shb_thp)) ||
	...
	if (gesture_status) {
		...
		if (cd->aod_support_on_tddi && cd->tp_ud_lowpower_status) {
			touch_driver_send_cmd(tdev->thp_core->sdev, &ic_sleep_cmd,
				sizeof(ic_sleep_cmd), NULL, 0);
	...
	u8 state)
{
	...
	ret = touch_driver_send_cmd(tdev->thp_core->sdev, &cmd, sizeof(cmd), NULL, 0);
