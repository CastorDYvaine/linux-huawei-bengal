...
static const struct nvt_ts_ic_info ic_info_table[] = {
	{
		...
		.ic_type = NVT36523N_SERIES,
		.addr = {0x3F0FE, 0x3F004, 0x2FD50, 0x2FD77}
	}
};
...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	rc = of_property_read_u32(nova_node, "support_vendor_ic_type",
...
static int touch_driver_ic_series_config(struct thp_device *tdev)
{
	...
	flag = (cd->support_vendor_ic_type == N9Z_SERIES) ||
		...
		(cd->support_vendor_ic_type == NVT36523N_SERIES);
	if (flag) {
	...
	uint8_t *r_buf, uint8_t *w_buf)
{
	...
	flag = ((tdev->thp_core->support_vendor_ic_type == NB1_SERIES) ||
		...
		(tdev->thp_core->support_vendor_ic_type == NVT36523N_SERIES)) &&
