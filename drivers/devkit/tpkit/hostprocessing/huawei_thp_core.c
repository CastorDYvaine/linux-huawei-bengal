...
static unsigned int g_spi_total_sync_count;
static unsigned int g_spi_sync_flag;
...
#define PROX_OFF_STATUS 1
#define RESUME_STATUS 2
...
static int thp_power_off_state_notify(u32 val);
#if (defined(CONFIG_HUAWEI_THP_MTK) || defined(CONFIG_HUAWEI_THP_QCOM))
...
struct ts_kit_ops thp_ops = {
	...
	.ts_power_off_state_notify = thp_power_off_state_notify,
};
...
#endif
#define MAX_TOTAL_SYNC_COUNT 10000000
int thp_spi_sync(struct spi_device *spi, struct spi_message *message)
{
	int ret;

	struct thp_core_data *cd = spi_get_drvdata(spi);

	if (g_spi_total_sync_count == MAX_TOTAL_SYNC_COUNT)
		g_spi_total_sync_count = 0;
	g_spi_total_sync_count++;
	g_spi_sync_flag++;
	if (cd->suspended)
		thp_log_info("%s:enter\n", __func__);
	else
	...
	int status, unsigned int delay_ms)
{
	...
	case THP_POWER_LDO:
		...
		/*
		 * Set load current at power-on
		 * The ldo_load parameter is configured
		 * based on the platform requirements. If
		 * the platform does not have requirements,
		 * the ldo_load parameter is optional.
		 */
		if (status && power->ldo_load)
			regulator_set_load(power->regulator, power->ldo_load);
...
static void not_support_prox_suspend(struct thp_core_data *cd)
{
...
static void support_prox_suspend(struct thp_core_data *cd)
{
	int status = 0;
	...
	thp_log_info("%s: early suspend! g_thp_prox_enable = %d %d\n",
	...
	if (cd->suspend_info.aod_and_proximity_support &&
		(cd->suspend_info.is_prox_suspend == PROX_OFF_STATUS)) {
		cd->need_work_in_suspend = THP_PROX_ENTER;
		thp_log_info("%s: need_work_in_suspend\n", __func__);
	}
...
static void aod_and_proximity_suspend(struct thp_core_data *cd)
{
	if (cd->suspend_info.is_prox_suspend == POWER_OFF_STATUS) {
		cd->need_work_in_suspend = 0;
		not_support_prox_suspend(cd);
	} else {
		support_prox_suspend(cd);
	}
}
...
static void thp_early_suspend(struct thp_core_data *cd)
{
	...
	if (cd->delay_work_for_pm) {
		...
		if (cd->proximity_support != PROX_SUPPORT)
			not_support_prox_suspend(cd);
		else if (cd->suspend_info.aod_and_proximity_support)
			aod_and_proximity_suspend(cd);
		else
			support_prox_suspend(cd);
	}
}
...
static void thp_after_resume(struct thp_core_data *cd)
{
	if (cd->delay_work_for_pm) {
		...
		if (cd->proximity_support != PROX_SUPPORT) {
			...
			if (cd->need_work_in_suspend) {
				thp_set_status(THP_STATUS_DAEMON_SLEEP, 0);
				thp_set_status(THP_STATUS_AFE_PROXIMITY,
					THP_PROX_EXIT);
				cd->daemon_flag = 0;
			} else {
			...
			if (cd->suspend_info.aod_and_proximity_support)
				cd->suspend_info.is_prox_suspend = RESUME_STATUS;
		}
	}
	if (cd->wait_afe_screen_on_support &&
	...
	enum lcd_kit_ts_pm_type pm_type)
{
	...
	case TS_SUSPEND_DEVICE:
		...
		thp_log_info("%s: suspend, spi_total_sync_count = %u, spi_sync_flag = %u\n",
			__func__, g_spi_total_sync_count, g_spi_sync_flag);
		thp_clean_fingers();
		break;
	...
	case TS_RESUME_DEVICE:
		if (cd->aod_support_on_tddi)
			thp_set_irq_status(cd, THP_IRQ_DISABLE);
		thp_log_info("%s: resume, spi_total_sync_count = %u, spi_sync_flag = %u\n",
			__func__, g_spi_total_sync_count, g_spi_sync_flag);
		if (cd->support_daemon_init_protect &&
			atomic_read(&(cd->fw_update_protect)) &&
	...
	case TS_2ND_POWER_OFF:
		...
		break;
	case TS_BLOCK_TPRST:
		cd->need_wait_lcd_rst = true;
		thp_log_info("%s:need wait lcd reset\n", __func__);
		break;
	case TS_UNBLOCK_TPRST:
		cd->need_wait_lcd_rst = false;
		thp_log_info("%s:lcd reset end\n", __func__);
		break;
	default:
		break;
	}
...
static long thp_spi_sync_get_data_size_from_user(const void __user *data,
...
static long thp_spi_sync_mem_init(u8 **tx_buf,
...
static long thp_spi_sync_data_with_user(unsigned int lock_status, u8 *tx_buf,
	u8 *rx_buf, struct thp_ioctl_spi_sync_data sync_data)
{
	int rc;
	struct thp_core_data *cd = thp_get_core_data();
	...
	unsigned int lock_status)
{
	...
	if (tui_enable)
		return 0;
#endif
	rc = thp_spi_sync_get_data_size_from_user(data, &sync_data, &sync_data_compat);
	if (rc)
		return rc;
	rc = thp_spi_sync_mem_init(&tx_buf, &rx_buf, sync_data);
	if (rc)
		goto exit;
	rc = thp_spi_sync_data_with_user(lock_status, tx_buf, rx_buf, sync_data);
	if (rc)
		goto exit;
	...
	unsigned int lock_status)
{
	...
	thp_log_info("%s spi communication in suspend\n", __func__);
	__pm_wakeup_event(cd->thp_wake_lock, jiffies_to_msecs(HZ));
	rc = thp_spi_sync_common(data, lock_status);
...
static long thp_ioctl_get_frame(unsigned long arg)
{
	struct thp_ioctl_get_frame_data data;
...
static long thp_ioctl_spi_sync_ssl_bl(unsigned long arg)
{
	struct thp_ioctl_spi_sync_data sync_data;
...
static long thp_ioctl_set_afe_status(unsigned long arg)
{
	int rc = 0;
...
static long thp_ioctl_cmd_spi_sync(unsigned long arg)
{
	return thp_ioctl_spi_sync((void __user *)arg, NEED_LOCK);
}

static long thp_ioctl_cmd_clear_frame_buffer(unsigned long arg)
{
	return thp_ioctl_clear_frame_buffer();
}

static long thp_ioctl_cmd_multiple_spi_xfer_sync(unsigned long arg)
{
	return thp_ioctl_multiple_spi_xfer_sync((void __user *)arg, NEED_LOCK);
}

static long thp_ioctl_cmd_spi_sync_no_lock(unsigned long arg)
{
	return thp_ioctl_spi_sync((void __user *)arg, DONOT_NEED_LOCK);
}

static long thp_ioctl_cmd_multiple_spi_xfer_sync_no_lock(unsigned long arg)
{
	return thp_ioctl_multiple_spi_xfer_sync((void __user *)arg, DONOT_NEED_LOCK);
}

static long thp_ioctl_cmd_spi_sync_in_suspend(unsigned long arg)
{
	return thp_ioctl_spi_sync_in_suspend((void __user *)arg, NEED_LOCK);
}

struct thp_ioctl_group {
	unsigned int cmd;
	long (*ioctl_func)(unsigned long arg);
};
	...
	unsigned long arg)
{
	...
	for (i = 0; i < ARRAY_SIZE(thp_ioctl_table); i++) {
		if (cmd == thp_ioctl_table[i].cmd) {
			ret = thp_ioctl_table[i].ioctl_func(arg);
			break;
		}
	}
...
static irqreturn_t irq_default_primary_handler(int irq, void *dev_id)
{
	...
	cd->report_cur_time.tv_usec = now.tv_nsec / 1000;
...
static int thp_chip_detect(struct thp_cmd_node *in_cmd)
{
	int ret = -EINVAL;
...
static void thp_update_unregister_ic_num(void)
{
	struct thp_core_data *cd = thp_get_core_data();
...
static int thp_check_register_status(struct thp_device *dev)
{
	int rc = -EINVAL;
	...
	register_flag = !cd->project_in_tp && cd->ic_name && dev->ic_name &&
			strcmp(cd->ic_name, dev->ic_name);
	if (register_flag) {
...
static int thp_parse_dts_config(void)
{
	int rc;
	struct thp_core_data *cd = thp_get_core_data();
	struct thp_device *dev = cd->thp_dev;
...
dev_init_err:
	cd->thp_dev = 0;
	thp_update_unregister_ic_num();
	return rc;
}

static int thp_dev_detect(struct thp_device *dev)
{
	int rc;
	struct thp_core_data *cd = thp_get_core_data();
...
int thp_register_dev(struct thp_device *dev)
{
	int rc = -EINVAL;
	struct thp_core_data *cd = thp_get_core_data();
	bool input_flag = (dev == NULL) || (cd == NULL);
	...
	rc = thp_check_register_status(dev);
	if (rc)
		goto register_err;
	...
	struct thp_core_data *cd)
{
	...
	if (!rc) {
		cd->check_tp_vendor = value;
		thp_log_info("%s:check_tp_vendor %u\n", __func__, value);
	}
	...
	cd->notify_daemon_release_lock = 0;
	rc = of_property_read_u32(thp_node, "notify_daemon_release_lock", &value);
	if (!rc) {
		cd->notify_daemon_release_lock = value;
		thp_log_info("%s:notify_daemon_release_lock %u\n", __func__, value);
	}
	return 0;
}
...
static void thp_prox_add_suspend(struct thp_core_data *cd, bool enable)
{
	...
	if (cd->reset_status_in_suspend_mode)
		/* for some tpic, no need to pull down reset in suspend */
...
static void thp_prox_add_poweroff(struct thp_core_data *cd, bool enable)
{
	...
	if ((enable == false) && (onetime_poweroff_done == false)) {
		...
		if (thp_prox_timeout_check(cd)) {
			if (cd->need_work_in_suspend && cd->notify_daemon_release_lock) {
...
static int thp_power_off_state_notify(u32 status)
{
	struct thp_core_data *cd = thp_get_core_data();
	...
	thp_log_info("%s :status value is %u\n", __func__, status);
	cd->suspend_info.is_prox_suspend = status;
	return 0;
}
...
#if (defined(CONFIG_HUAWEI_THP_MTK) || defined(CONFIG_HUAWEI_THP_QCOM))
static int ts_suspend_func(struct thp_cmd_node *proc_cmd)
{
	if (proc_cmd == NULL) {
		thp_log_err("%s:null pointer\n", __func__);
		return -EINVAL;
...
static int ts_resume_func(struct thp_cmd_node *proc_cmd)
{
	if (proc_cmd == NULL) {
		thp_log_err("%s:null pointer\n", __func__);
		return -EINVAL;
...
#if defined CONFIG_LCD_KIT_DRIVER
static int thp_multi_resume_func(struct thp_cmd_node *proc_cmd)
{
	if (proc_cmd == NULL) {
		thp_log_err("%s:null pointer\n", __func__);
		return -EINVAL;
...
static int thp_multi_suspend_func(struct thp_cmd_node *proc_cmd)
{
	if (proc_cmd == NULL) {
		thp_log_err("%s:null pointer\n", __func__);
		return -EINVAL;
...
static int tskit_multi_resume_func(struct thp_cmd_node *proc_cmd)
{
	if (proc_cmd == NULL) {
		thp_log_err("%s:null pointer\n", __func__);
		return -EINVAL;
...
static int tskit_multi_suspend_func(struct thp_cmd_node *proc_cmd)
{
	if (proc_cmd == NULL) {
		thp_log_err("%s:null pointer\n", __func__);
		return -EINVAL;
...
static int ts_screen_fold_func(struct thp_cmd_node *proc_cmd)
{
	if (proc_cmd == NULL) {
		thp_log_err("%s:null pointer\n", __func__);
		return -EINVAL;
...
static int ts_screen_off_fold_func(struct thp_cmd_node *proc_cmd)
{
	if (proc_cmd == NULL) {
		thp_log_err("%s:null pointer\n", __func__);
		return -EINVAL;
...
static int ts_screen_unfold_func(struct thp_cmd_node *proc_cmd)
{
	if (proc_cmd == NULL) {
		thp_log_err("%s:null pointer\n", __func__);
		return -EINVAL;
...
struct thp_proc_command_group {
	enum thp_cmd cmd;
	int  (*func)(struct thp_cmd_node *proc_cmd);
};

struct thp_proc_command_group thp_proc_cmd_table[] = {
	{TS_CHIP_DETECT, thp_chip_detect},
#if (defined(CONFIG_HUAWEI_THP_MTK) || defined(CONFIG_HUAWEI_THP_QCOM))
	{TS_SUSPEND, ts_suspend_func},
	{TS_RESUME, ts_resume_func},
#endif
#if defined CONFIG_LCD_KIT_DRIVER
	{THP_MUTIL_RESUME_THREAD, thp_multi_resume_func},
	{THP_MUTIL_SUSPEND_THREAD, thp_multi_suspend_func},
	{TSKIT_MUTIL_RESUME_THREAD, tskit_multi_resume_func},
	{TSKIT_MUTIL_SUSPEND_THREAD, tskit_multi_suspend_func},
	{TS_SCREEN_FOLD, ts_screen_fold_func},
	{TS_SCREEN_OFF_FOLD, ts_screen_off_fold_func},
	{TS_SCREEN_UNFOLD, ts_screen_unfold_func},
#endif
};
...
static int thp_proc_command(struct thp_cmd_node *cmd)
{
	...
	while (true) {
		...
		for (i = 0; i < ARRAY_SIZE(thp_proc_cmd_table); i++) {
			if (proc_cmd->command == thp_proc_cmd_table[i].cmd) {
				error = thp_proc_cmd_table[i].func(proc_cmd);
				break;
			}
		}
