	...
	struct lcd_kit_backlight_info *pbl_config)
{
	...
	if (prop->length !=
		(int)(tmp_val * BACKLIGHT_IC_CMD_LEN * sizeof(unsigned int))) {
	...
	unsigned char *pos_votage, unsigned char *neg_votage)
{
	...
	for (i = 0; i < ARRAY_SIZE(backlight_bias_config); i++) {
		...
		for (j = 0; j < backlight_bias_config[i].len; j++) {
			if (backlight_bias_config[i].vol_table[j].voltage == (unsigned int)vsp) {
		...
		for (j = 0; j < backlight_bias_config[i].len; j++) {
			if (backlight_bias_config[i].vol_table[j].voltage == (unsigned int)vsn) {
...
static int lcd_kit_backlight_set_bias(int vpos, int vneg)
{
	...
	if (g_vol_mapped == BACKLIGHT_FALSE) {
		...
		if (pbacklight_dev->bl_config.init_update_boost) {
			boost_index = pbacklight_dev->bl_config.init_boost_index;
			pbacklight_dev->bl_config.init_cmds[boost_index].cmd_val = vsp_vol +
				(unsigned char)pbacklight_dev->bl_config.boost_offset;
		}
...
int lcd_kit_set_backlight(unsigned int bl_level)
{
	...
	if (pbacklight_dev->bl_config.bl_10000_support) {
		level = (unsigned int)bl_lvl_map(bl_level);
	} else {
...
static bool lcd_kit_backlight_dual_ic_check(void)
{
	...
	if (val != pbacklight_dev->bl_config.init_cmds[0].cmd_val) {
		LCD_KIT_ERR("reg = 0x%x, cmd_val = 0x%x, val = 0x%x\n",
			__func__, pbacklight_dev->bl_config.init_cmds[0].cmd_reg, val);
		return false;
	}
	return true;
}
