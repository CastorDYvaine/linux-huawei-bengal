...
static struct delayed_work lcd_kit_poweroff_work;
bool poweroff_work_enable = 0;
#endif
...
int lcd_kit_panel_set_vint2_cmd(struct dsi_panel *panel, enum dsi_cmd_set_type type)
{
	int rc = 0;
	...
	if (rc)
		LCD_KIT_ERR("[%s] failed to send vint2 cmds type:%d, rc=%d\n", panel->name, rc, type);
...
#ifdef CONFIG_HW_COMB_KEY
static inline void lcd_kit_poweroff_work_fn(struct work_struct *work)
{
	LCD_KIT_INFO("enter\n");
...
int lcd_kit_poweroff_notifier_call(
	struct notifier_block *powerkey_nb, unsigned long event,
	void *data)
{
	if (event == POWER_KEY_PRESS_DOWN) {
		schedule_delayed_work(&lcd_kit_poweroff_work, POWER_PRESS_SEC * HZ);
		poweroff_work_enable = 1;
	} else {
		if (poweroff_work_enable) {
			cancel_delayed_work(&lcd_kit_poweroff_work);
			poweroff_work_enable = 0;
		}
	}
	LCD_KIT_INFO("exit\n");
	return 0;
...
static struct notifier_block lcd_kit_poweroff_flush_nb = {
	.notifier_call = lcd_kit_poweroff_notifier_call,
	.priority = 0,
};
#endif
...
static int32_t poweric_gpio_detect(uint32_t panel_id, int32_t *out)
{
	...
	gpio_free(detect_gpio);
	if (gpio_value != (int32_t)(disp_info->elvdd_detect.exp_value))
		ret = LCD_KIT_FAIL;
	*out = gpio_value;
...
void poweric_detect_delay(uint32_t panel_id)
{
	...
	if (disp_info->elvdd_detect.is_start_delay_timer == false) {
		...
		poweric_det_delay.timer._softexpires = (ktime_t)(jiffies +
	...
	unsigned int level)
{
	...
	uint32_t panel_id;
	unsigned int tuned_level = level;
	...
	if (panel->power_mode == SDE_MODE_DPMS_LP1) {
		...
		if (level > 0)
			LCD_KIT_ERR("Poweron backlight update failed\n");
		return ret;
	}
	...
	struct drm_file *file_priv)
{
	...
	param->width = (int)lcd_kit_info[PRIMARY_PANEL].xres;
	param->height = (int)lcd_kit_info[PRIMARY_PANEL].yres;
	param->maxluminance = lcd_kit_info[PRIMARY_PANEL].maxluminance;
	param->minluminance = lcd_kit_info[PRIMARY_PANEL].minluminance;
	param->maxbacklight = (int)lcd_kit_info[PRIMARY_PANEL].bl_max;
	param->minbacklight = (int)lcd_kit_info[PRIMARY_PANEL].bl_min;
...
int lcd_kit_panel_post_unprepare(struct dsi_panel *panel)
{
	...
	panel_info = lcm_get_panel_info(panel_id);
	mutex_lock(&panel->panel_lock);
	if (disp_info->alpm.noprox_support && common_ops->set_proximity_flag) {
		proximity_flag = panel_info->idle_type == DISPLAY_IDLE_PROX_OFF ? 1 : 0;
		common_ops->set_proximity_flag(proximity_flag);
	}
...
int lcd_kit_init(struct dsi_panel *panel)
{
	...
	if ((panel_id == SECONDARY_PANEL) &&
		(lcd_kit_get_product_type() == LCD_SINGLE_DSI_MULTIPLEX_TYPE))
		lcm_set_panel_state(panel_id, LCD_POWER_STATE_OFF);
	else
		lcm_set_panel_state(panel_id, LCD_POWER_STATE_ON);
...
#endif
#ifdef CONFIG_HW_COMB_KEY
	if (disp_info->pwrkey_long_press_support) {
		INIT_DELAYED_WORK(
			&lcd_kit_poweroff_work, lcd_kit_poweroff_work_fn);
		comb_key_register_notifier(&lcd_kit_poweroff_flush_nb);
	}
#endif
#ifdef CONFIG_LCD_KIT_HYBRID
	...
	struct drm_file *file_priv)
{
	...
	if (mmi_level == 0) {
		...
		mmi_level = (int)fp_recovery_level;
...
static void report_dsi_tx_reboot_err(void)
{
#if defined CONFIG_HUAWEI_DSM
	if (lcd_dclient && !dsm_client_ocuppy(lcd_dclient)) {
		dsm_client_record(lcd_dclient, "DSI command transfer failed reboot\n");
		dsm_client_notify(lcd_dclient, DSM_LCD_MIPI_ERROR_NO);
	}
#endif
}

#define ERROR_TIME 11
#define DMD_TIME 6
void lcd_kit_trigger_reboot(void)
{
	...
	if (pinfo->need_reboot != 0)
		pinfo->need_reboot = pinfo->need_reboot + 1;
	if (pinfo->need_reboot == DMD_TIME)
		report_dsi_tx_reboot_err();
	if (pinfo->need_reboot == ERROR_TIME)
		BUG_ON(1);
}
...
void lcd_kit_crtc_info_store(struct drm_plane *plane)
{
	...
	if (((uint32_t)plane->state->crtc_y + plane->state->crtc_h) > common_info->fold_info.crtc_h)
		common_info->fold_info.crtc_h = ((uint32_t)plane->state->crtc_y + plane->state->crtc_h);
}
...
int panel_drm_hbm_set_for_fingerprint(bool is_enable)
{
	...
	int backlight_type = -1;
	struct qcom_panel_info *panel_info = NULL;
	int hbm_level = (int)display_engine_get_hbm_level(DISPLAY_ENGINE_PANEL_INNER);
	if (hbm_level < 0) {
	...
	if (is_enable) {
		...
		backlight_type = BACKLIGHT_LOW_LEVEL;
		hbm_level = (int)fp_recovery_level;
	}
