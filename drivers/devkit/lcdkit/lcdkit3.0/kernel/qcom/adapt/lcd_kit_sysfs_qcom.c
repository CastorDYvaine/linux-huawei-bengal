...
static int lcd_get_2d_barcode(uint32_t panel_id, char *oem_data)
{
	...
	if (disp_info->oeminfo.barcode_2d.support) {
		...
		oem_data[1] = (char)(BARCODE_BLOCK_NUM +
...
static ssize_t lcd_display_idle_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct qcom_panel_info *panel_info = NULL;
	uint32_t panel_id = lcd_get_active_panel_id();
	...
	return (ssize_t)snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%d\n", panel_info->idle_type);
}

static ssize_t lcd_display_idle_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct qcom_panel_info *panel_info = NULL;
	uint32_t panel_id = lcd_get_active_panel_id();
	...
	if (!sscanf_s(buf, "%u", &panel_info->idle_type)) {
		LCD_KIT_ERR("sscanf return invaild\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
...
struct lcd_kit_sysfs_ops g_lcd_sysfs_ops = {
	...
	.display_idle_mode_show = lcd_display_idle_mode_show,
	.display_idle_mode_store = lcd_display_idle_mode_store,
#ifdef CONFIG_MATTING_ALGO_TASK
