...
struct mask_delay_time {
	...
	/* DC Delay Threshold */
	uint8_t dc_delay_support;
	uint64_t dc_left_time_to_te_us;
	uint64_t dc_right_time_to_te_us;
	uint32_t dc_delay_time_before_fp;
	uint32_t dc_delay_time_after_fp;
};
...
struct qcom_panel_info {
	...
	struct work_struct vint2_exit_before_cmd_work;
	struct work_struct vint2_exit_after_cmd_work;
#ifdef CONFIG_MATTING_ALGO_TASK
