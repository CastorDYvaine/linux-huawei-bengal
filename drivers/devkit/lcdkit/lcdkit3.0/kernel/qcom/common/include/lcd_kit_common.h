...
struct lcd_kit_common_ops {
	...
	int (*hbm_set_level)(uint32_t panel_id, void *hld, unsigned int hbm_level);
	...
	int (*set_vint2_cmd)(uint32_t panel_id, int type, int async);
	int (*set_proximity_flag)(int flag);
};
...
struct lcd_kit_common_info {
	...
	u32 panel_pt_always_keep_reset_high;
	/* effect */
	int bl_level_max;
	int bl_level_min;
...
struct lcd_kit_power_desc {
	...
	struct lcd_kit_array_data lcd_te_switch;
};
