...
ssize_t lcd_kit_ddic_lv_detect_test_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
...
ssize_t lcd_kit_ddic_lv_detect_test_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret;
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
...
static ssize_t lcd_kit_finger_unlock_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	if (sysfs_ops->finger_unlock_show)
		ret = sysfs_ops->finger_unlock_show(dev, attr, buf);
	return ret;
}

static ssize_t lcd_kit_finger_unlock_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	if (!sysfs_ops->finger_unlock_store) {
		LCD_KIT_ERR("finger_unlock_store is NULL!\n");
		return LCD_KIT_FAIL;
	}
	if (sysfs_ops->finger_unlock_store(dev, attr, buf, count)) {
		LCD_KIT_ERR("finger_unlock_store error!\n");
		return LCD_KIT_FAIL;
	}
	return count;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (sysfs_ops->display_idle_mode_show)
		ret = sysfs_ops->display_idle_mode_show(dev, attr, buf);
	return ret;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sysfs_ops->display_idle_mode_store)
		ret = sysfs_ops->display_idle_mode_store(dev, attr, buf, count);
	return count;
}
...
static DEVICE_ATTR(lcd_fps_order, 0644, lcd_kit_fps_order_show,
	NULL);
static DEVICE_ATTR(panel_sncode, 0644, lcd_kit_panel_sncode_show,
	lcd_kit_panel_sncode_store);
static DEVICE_ATTR(finger_unlock_state, 0644, lcd_kit_finger_unlock_show,
	lcd_kit_finger_unlock_store);
...
struct attribute *lcd_kit_conf[] = {
	...
	&dev_attr_finger_unlock_state.attr,
