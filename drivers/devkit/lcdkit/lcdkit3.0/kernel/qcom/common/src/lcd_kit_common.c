...
static void lcd_kit_panel_parse_backlight(uint32_t panel_id, struct device_node *np);
static void lcd_kit_panel_parse_vint2_cmd(uint32_t panel_id, const struct device_node *np);
...
int lcd_kit_dbg_get_dbg_level(void)
{
	return lcd_kit_msg_level;
}

void lcd_kit_dbg_set_dbg_level(int level)
{
...
int lcd_kit_set_mipi_switch_ctrl(uint32_t panel_id, int enable)
{
	...
	case GPIO_MODE:
		if (enable) {
			if (adapt_ops->gpio_enable)
				adapt_ops->gpio_enable(panel_id, LCD_KIT_MIPI_SWITCH);
		} else {
			if (adapt_ops->gpio_disable)
				adapt_ops->gpio_disable(panel_id, LCD_KIT_MIPI_SWITCH);
		}
		break;
	case NONE_MODE:
...
int lcd_kit_set_te_switch_ctrl(uint32_t panel_id, int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	adapt_ops = lcd_kit_get_adapt_ops();
	...
	if (power_hdl->lcd_te_switch.buf == NULL) {
		LCD_KIT_ERR("can not get lcd mipi switch!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_te_switch.buf[0]) {
	case GPIO_MODE:
		if (enable) {
			if (adapt_ops->gpio_enable)
				adapt_ops->gpio_enable(panel_id, LCD_KIT_TE_SWITCH);
		} else {
			if (adapt_ops->gpio_disable)
				adapt_ops->gpio_disable(panel_id, LCD_KIT_TE_SWITCH);
		}
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd te switch is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd te switch is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}
	...
	uint32_t event, uint32_t data, uint32_t delay)
{
	...
	case EVENT_BLOCK_TS:
		...
		break;
	case EVENT_TE_SWITCH:
		ret = lcd_kit_set_te_switch_ctrl(panel_id, data);
		break;
	default:
...
static int lcd_kit_hbm_set_level(uint32_t panel_id, void *hld, unsigned int level)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	int hbm_dimming, int hbm_level, int fps_status)
{
	...
	common_info->hbm.hbm_level_current = (u32)hbm_level;
	...
	if (hbm_level > 0) {
		if (last_hbm_level == 0) {
		 ...
		 /* set hbm level */
		lcd_kit_hbm_set_level(panel_id, hld, (unsigned int)hbm_level);
	} else {
		if (last_hbm_level == 0) {
			/* disable dimming */
...
static int lcd_kit_set_vint2_cmd(uint32_t panel_id, int type, int async)
{
	struct lcd_kit_ops *lcd_ops = NULL;
	int ret = 0;
	...
	if (lcd_ops->vint2_cmd)
		ret = lcd_ops->vint2_cmd(panel_id, type, async);
	return ret;
}
...
static void lcd_kit_panel_parse_effect(uint32_t panel_id, struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,panel-pt-always-keep-reset-high",
		&common_info->panel_pt_always_keep_reset_high, 1);
...
static void lcd_kit_proximity_parse(uint32_t panel_id, struct device_node *np)
{
	...
	if (common_info->thp_proximity.support) {
		...
		lcd_kit_parse_u32(np, "lcd-kit,proximity-reset-delay-min",
			&common_info->thp_proximity.after_reset_delay_min, 0);
		lcd_kit_parse_u32(np, "lcd-kit,proximity-power-off-delay-min",
			&common_info->thp_proximity.power_off_delay_min, 0);
...
static void lcd_kit_panel_parse_util(uint32_t panel_id, struct device_node *np)
{
	...
	lcd_kit_panel_parse_vint2_cmd(panel_id, np);
...
static void lcd_kit_panel_parse_vint2_cmd(uint32_t panel_id, const struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,hbm-exit-process-vint2-support",
		&common_info->process_vint2_support, 0);
}
...
static void lcd_kit_parse_tddi_aod(uint32_t panel_id, struct device_node *np)
{
	...
	if (common_info->tddi_aod_support) {
		...
		lcd_kit_parse_u32(np, "lcd-kit,lcdkit-aod-doze-time-delay",
			&common_info->aod_doze_time_delay, 0);
	}
}
...
static void lcd_kit_parse_power(uint32_t panel_id, struct device_node *np)
{
	...
	if (power_hdl->lcd_te_switch.buf == NULL)
		lcd_kit_parse_array_data(np, "lcd-kit,lcd-te-switch",
			&power_hdl->lcd_te_switch);
}
...
void lcd_kit_delay(int wait, int waittype, bool allow_sleep)
{
	...
	if (waittype == LCD_KIT_WAIT_US) {
		...
		if (wait > 10 && allow_sleep)
			...
			mdelay((unsigned int)wait);
	} else {
		if (allow_sleep)
			msleep((unsigned int)wait * 1000);
		else
			mdelay((unsigned int)wait * 1000);
	}
}

int lcd_kit_set_proximity_flag(int flag)
{
	struct ts_kit_ops *ts_ops = NULL;
	int ret = LCD_KIT_OK;
	ts_ops = ts_kit_get_ops();
	if (!ts_ops || !ts_ops->ts_power_off_state_notify) {
		LCD_KIT_ERR("ts_kit_ops  input is null\n");
		return LCD_KIT_FAIL;
	}
	ret = ts_ops->ts_power_off_state_notify(flag);
	return LCD_KIT_OK;
}
...
struct lcd_kit_common_ops g_lcd_kit_common_ops = {
	...
	.set_vint2_cmd= lcd_kit_set_vint2_cmd,
	.set_proximity_flag = lcd_kit_set_proximity_flag,
};
