...
static struct display_engine_common_context g_de_common_context = {
	/* {0, 0, 1176, 2480} is PAL primary rectangle */
	.compensation_data.roi_param.top = 0,
	.compensation_data.roi_param.bottom = 1176,
...
static u32 g_fingerprint_hbm_level = MAX_DBV;
static u32 g_fingerprint_backlight_type = BACKLIGHT_LOW_LEVEL;
static u32 g_fingerprint_backlight_type_real = BACKLIGHT_LOW_LEVEL;
...
static void get_sync_time(struct mask_delay_time *mask_delay, uint64_t real_time,
	uint64_t *left_thres_us, uint64_t *right_thres_us, bool is_dc)
{
	uint64_t left_time_cache;
	uint64_t right_time_cache;

	if (is_dc && mask_delay->dc_delay_support > 0) {
		left_time_cache = mask_delay->dc_left_time_to_te_us;
		right_time_cache = mask_delay->dc_right_time_to_te_us;
	} else {
		left_time_cache = mask_delay->left_time_to_te_us;
		right_time_cache = mask_delay->right_time_to_te_us;
	}
	*left_thres_us = correct_time_based_on_fps(real_time, left_time_cache);
	*right_thres_us = correct_time_based_on_fps(real_time, right_time_cache);

	LCD_KIT_INFO("dc_support = %lu, is_dc = %lu, dc_left_time = %lu,"
		" dc_right_time = %lu, left_time = %lu,right_time = %lu,"
		" te_interval = %lu\n", mask_delay->dc_delay_support, is_dc,
		mask_delay->dc_left_time_to_te_us, mask_delay->dc_right_time_to_te_us,
		mask_delay->left_time_to_te_us, mask_delay->right_time_to_te_us,
		mask_delay->te_interval_us);
}

static uint64_t get_backlight_sync_delay_time_us(struct qcom_panel_info *panel_info,
	struct display_engine_context *de_context, uint32_t panel_id, bool is_dc)
{
	...
	get_sync_time(&panel_info->mask_delay, real_te_interval, &left_thres_us,
		&right_thres_us, is_dc);

	if (te_interval == 0) {
		LCD_KIT_ERR("te_interval is 0, used default time\n");
		te_interval = TE_INTERVAL_60_FPS_US;
	}
	diff_from_te_time = (uint64_t)(ktime_to_us(ktime_get()) -
	...
	if (diff_from_te_time > real_te_interval) {
		...
		diff_from_te_time = (uint64_t)(ktime_to_us(ktime_get()) - ktime_to_us(
...
static void handle_backlight_hbm_and_dc_delay(struct qcom_panel_info *panel_info,
	uint32_t panel_id, bool is_entering, bool is_dc)
{
	uint32_t mask_delay_time_cache;
	uint32_t mask_delay_time;
	uint32_t real_te_interval = get_real_te_interval(panel_id);

	if (is_dc && panel_info->mask_delay.dc_delay_support > 0) {
		mask_delay_time_cache = is_entering ?
			panel_info->mask_delay.dc_delay_time_before_fp :
			panel_info->mask_delay.dc_delay_time_after_fp;
	} else {
		mask_delay_time_cache = is_entering ? panel_info->mask_delay.delay_time_before_fp :
			panel_info->mask_delay.delay_time_after_fp;
	}
	mask_delay_time = correct_time_based_on_fps(real_te_interval, mask_delay_time_cache);
	LCD_KIT_INFO("real_te_interval:%d, delay_time_before_fp:%d,"
		" delay_time_after_fp:%d, mask_delay_time:%d, enter is %lu\n",
		real_te_interval, panel_info->mask_delay.delay_time_before_fp,
		panel_info->mask_delay.delay_time_after_fp, mask_delay_time, is_entering);
...
int display_engine_fp_backlight_set_sync(uint32_t panel_id, u32 level, u32 backlight_type)
{
	...
	if (g_fingerprint_backlight_type == BACKLIGHT_LOW_LEVEL &&
		lcd_common_info->process_vint2_support)
		lcd_kit_get_common_ops()->set_vint2_cmd(panel_id, DSI_CMD_SET_VINT2_EXIT_BEFORE,
			LCD_KIT_VINT2_CMD_SYNC);
	display_engine_brightness_update_backlight();
	if (g_fingerprint_backlight_type == BACKLIGHT_LOW_LEVEL &&
		lcd_common_info->process_vint2_support)
		lcd_kit_get_common_ops()->set_vint2_cmd(panel_id, DSI_CMD_SET_VINT2_EXIT_AFTER,
			LCD_KIT_VINT2_CMD_ASYNC);

	handle_backlight_hbm_and_dc_delay(panel_info, panel_id,
		g_fingerprint_backlight_type == BACKLIGHT_HIGH_LEVEL, false);
	g_fingerprint_backlight_type_real = g_fingerprint_backlight_type;
	return LCD_KIT_OK;
}
...
static int get_mapped_alpha_inner_handle_mode(unsigned int bl_level,
	struct display_engine_context *de_context)
{
	...
	case DISPLAY_ENGINE_BRIGHTNESS_MODE_HIGH_PRECISION:
		...
		LCD_KIT_DEBUG("brightness.mode_set_by_ioctl[%u], in_level[%u] -> out_alpha[%d]\n",
...
static int get_mapped_alpha_inner(unsigned int bl_level)
{
	struct display_engine_context *de_context = get_de_context(DISPLAY_ENGINE_PANEL_INNER);
	...
	if (bl_level < 0 || bl_level >= BRIGHTNESS_LUT_LENGTH) {
		LCD_KIT_ERR("invalid bl_level %u\n", bl_level);
		return ALPHA_DEFAULT;
	}
...
static bool handle_brightness_lut(
	unsigned int in_level, struct display_engine_context *de_context, unsigned int *out_level, int mode_in_use)
{
	...
	*out_level = (unsigned int)to_dbv_lut[in_level];
	LCD_KIT_DEBUG("brightness.mode_in_use [%u], in_level [%u] -> out_level [%u]\n",
		mode_in_use, in_level, *out_level);
	return true;
}

static unsigned int get_mapped_level_handle_mode(unsigned int in_level, struct display_engine_context *de_context)
{
	unsigned int out_level = in_level;
	unsigned int mode_in_use;
...
unsigned int display_engine_brightness_get_mapped_level(unsigned int in_level, uint32_t panel_id)
{
	struct display_engine_context *de_context = get_de_context(DISPLAY_ENGINE_PANEL_INNER);
	...
	if (in_level < 0 || in_level >= BRIGHTNESS_LUT_LENGTH) {
		LCD_KIT_ERR("invalid in_level %u\n", in_level);
		goto error_out;
	}
	if (!de_context->is_inited) {
		LCD_KIT_WARNING("display engine is not inited yet.\n");
...
int display_engine_brightness_get_mapped_alpha(void)
{
	...
	de_context->brightness.last_alpha = de_context->brightness.current_alpha;
	de_context->brightness.current_alpha = (unsigned int)alpha;
	return alpha;
}
	...
	struct display_engine_brightness_lut *input_lut)
{
	...
	ret1 = copy_from_user(target_lut->to_dbv_lut, input_lut->to_dbv_lut,
		BRIGHTNESS_LUT_LENGTH * sizeof(__u16));
	if (ret1 != 0) {
		LCD_KIT_ERR("[effect] copy dbv lut failed, ret = %d\n", ret1);
		return LCD_KIT_FAIL;
	}
	ret1 = copy_from_user(target_lut->to_alpha_lut, input_lut->to_alpha_lut,
		BRIGHTNESS_LUT_LENGTH * sizeof(__u16));
	if (ret1 != 0) {
	...
	struct display_engine_alpha_map *input_alpha_map)
{
	...
	ret1 = copy_from_user(target_alpha_map->lum_lut, input_alpha_map->lum_lut,
	...
	struct display_engine_context *de_context)
{
	...
	sync_delay_time_us = get_backlight_sync_delay_time_us(
		panel_info, de_context, PRIMARY_PANEL, true);
	...
	struct display_engine_panel_info *info)
{
	...
	info->max_luminance = (unsigned int)panel_info->maxluminance;
	info->min_luminance = (unsigned int)panel_info->minluminance;
	...
	strncpy(info->panel_version, panel_info->panel_version.lcd_panel_version,
		PANEL_VERSION_LEN - 1);
	strncpy(info->panel_name, lcd_common_info->panel_name, PANEL_NAME_LEN - 1);
	info->local_hbm_support = (int)display_engine_local_hbm_get_support();
...
static uint32_t calculate_acc_value(uint32_t type, uint32_t region, struct timeval cur_time)
{
	...
	delta = level * (uint64_t)((cur_time.tv_sec - last_time.tv_sec) * 1000 +
	...
	struct display_engine_roi_param roi_param)
{
	...
	/* The BIC rect will be offset 8 pixel,right-left is always full-screen,no need to change it */
	if (roi_param.bottom >= 8 && roi_param.bottom >= roi_param.top + 8) {
		roi_param.bottom = roi_param.bottom - 8;
		LCD_KIT_DEBUG("DE_FC bottom is set to %d\n", roi_param.bottom);
	} else {
		LCD_KIT_ERR("DE_FC dbi_roi_set fail, please check bottom %d\n", roi_param.bottom);
		return LCD_KIT_FAIL;
	}
	...
	struct display_engine_foldable_info *param)
{
	...
	for (color = 0; color < DE_COLOR_INDEX_COUNT; color++) {
		ret1 = copy_from_user(dbi_weight[color], param->lut[color],
	...
	if (grayscale <= (int)gamma_info->node_grayscale.buf[0]) {
		...
		*index = cnt - 1;
		...
		for (i = 0; i < cnt; i++) {
			...
			if (grayscale <= (int)gamma_info->node_grayscale.buf[i]) {
				*index = i;
				break;
			}
		}
	...
	struct color_cmds_rgb *color_cmds)
{
	...
	common_info->hbm.circle_color_cmds.cmds[color_cmd_index].payload[CMDS_RED_HIGH] =
		(char)color_cmds->red_payload[0];
	common_info->hbm.circle_color_cmds.cmds[color_cmd_index].payload[CMDS_RED_LOW] =
		(char)color_cmds->red_payload[1];
	common_info->hbm.circle_color_cmds.cmds[color_cmd_index].payload[CMDS_GREEN_HIGH] =
		(char)color_cmds->green_payload[0];
	common_info->hbm.circle_color_cmds.cmds[color_cmd_index].payload[CMDS_GREEN_LOW] =
		(char)color_cmds->green_payload[1];
	common_info->hbm.circle_color_cmds.cmds[color_cmd_index].payload[CMDS_BLUE_HIGH] =
		(char)color_cmds->blue_payload[0];
	common_info->hbm.circle_color_cmds.cmds[color_cmd_index].payload[CMDS_BLUE_LOW] =
		(char)color_cmds->blue_payload[1];
	...
	struct display_engine_param *param)
{
	...
	uint32_t alpha = (uint32_t)param->local_hbm.test.alpha;
	int ret;
...
void display_engine_handle_power_mode_exit(int mode)
{
	...
	de_context->brightness.power_mode = (u32)mode;
