...
static int rpmstats_power(struct platform_device *pdev,
				struct msm_rpmstats_platform_data *pd)
{
    struct kobject *rpmstats_kobj_power = NULL;
    struct msm_rpmstats_kobj_attr *rpms_ka_power = NULL;
    int ret = 0;

    rpmstats_kobj_power = kobject_create_and_add("system_sleep_power", power_kobj);
    if (!rpmstats_kobj_power) {
        pr_err("Cannot create rpmstats kobject\n");
        ret = -ENOMEM;
        goto fail;
    }

    rpms_ka_power = kzalloc(sizeof(*rpms_ka_power), GFP_KERNEL);
    if (!rpms_ka_power) {
        kobject_put(rpmstats_kobj_power);
        ret = -ENOMEM;
        goto fail;
    }

    rpms_ka_power->kobj = rpmstats_kobj_power;

    sysfs_attr_init(&rpms_ka_power->ka.attr);
    rpms_ka_power->pd = pd;
    rpms_ka_power->ka.attr.mode = 0444;
    rpms_ka_power->ka.attr.name = "stats";
    rpms_ka_power->ka.show = rpmstats_show;
    rpms_ka_power->ka.store = NULL;

    ret = sysfs_create_file(rpmstats_kobj_power, &rpms_ka_power->ka.attr);
    platform_set_drvdata(pdev, rpms_ka_power);
