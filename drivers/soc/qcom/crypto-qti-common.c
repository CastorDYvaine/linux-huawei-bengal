			       ...
			       u8 data_unit_mask, int capid)
{
	...
	if (err1) {
		...
		err2 = crypto_qti_invalidate_key(ice_entry, slot, key);
#else
...
#ifdef CONFIG_DISK_MAGO
int crypto_qti_keyslot_evict(void *priv_data, unsigned int slot, const struct blk_crypto_key *key)
#else
...
#ifdef CONFIG_DISK_MAGO
	err = crypto_qti_invalidate_key(ice_entry, slot, key);
#else
