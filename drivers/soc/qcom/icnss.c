...
#ifdef CONFIG_HUAWEI_MODEM_CRASH_LOG
	wpss_reset_save_log(reason, sizeof(reason));
#endif
...
#ifdef CONFIG_HUAWEI_MODEM_CRASH_LOG
	sprintf(reason, "Modem down, state:0x%lx crash:%d pd_crash %d pd_shutdown %d",
		priv->state,
		notif->crashed,
		priv->stats.recovery.root_pd_crash,
		priv->stats.recovery.root_pd_shutdown);
...
#ifdef CONFIG_HUAWEI_MODEM_CRASH_LOG
	sprintf(reason, "PD service down, pd_state: %d, state: 0x%lx: cause: %s\n",
		*state, priv->state, icnss_pdr_cause[cause]);
	wpss_reset_save_log(reason, MAX_SSR_REASON_LEN);
#endif
