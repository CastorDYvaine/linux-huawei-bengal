...
static void mceusb_gen1_init(struct mceusb_dev *ir)
{
	 ...
	 */
	ret = usb_control_msg_recv(ir->usbdev, 0,
			      USB_REQ_SET_ADDRESS, USB_DIR_IN | USB_TYPE_VENDOR, 0, 0,
			      data, USB_CTRL_MSG_SZ, HZ * 3, GFP_KERNEL);
	...
	ret = usb_control_msg_send(ir->usbdev, 0,
			      ...
			      0xc04e, 0x0000, NULL, 0, HZ * 3, GFP_KERNEL);
	...
	ret = usb_control_msg_send(ir->usbdev, 0,
			      4, USB_TYPE_VENDOR,
			      0x0808, 0x0000, NULL, 0, HZ * 3, GFP_KERNEL);
	dev_dbg(dev, "set char length - retB = %d", ret);

	/* bRequest 2: set handshaking to use DTR/DSR */
	ret = usb_control_msg_send(ir->usbdev, 0,
			      2, USB_TYPE_VENDOR,
			      0x0000, 0x0100, NULL, 0, HZ * 3, GFP_KERNEL);
