...
#ifdef CONFIG_HUAWEI_BATTERY_BENGAL_ISCD
static int is_alarm_start = 0;
#endif
...
static void iscd_notifier_call_event_handler(int event_type)
{
	if (!g_iscd_info) {
		hwlog_err("di is null, can't do iscd detect\n");
		return;
	}
	switch (event_type) {
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_CHARGING_START:
		iscd_notifier_call_event_handler(ISCD_START_CHARGE_EVENT);
		break;
	case POWER_NE_CHARGING_STOP:
		iscd_notifier_call_event_handler(ISCD_STOP_CHARGE_EVENT);
		break;
	default:
		break;
	}
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_CHG_CHARGING_DONE:
		iscd_notifier_call_event_handler(ISCD_CHARGE_DONE_EVENT);
		break;
	case POWER_NE_CHG_CHARGING_RECHARGE:
		iscd_notifier_call_event_handler(ISCD_RECHARGE_EVENT);
		break;
	default:
		break;
	}
...
static int iscd_charge_thirdplat_event_notifier_call(struct notifier_block *nb,
	unsigned long event, void *data)
{
	switch (event) {
	case POWER_NE_THIRDPLAT_CHARGING_DONE:
		iscd_notifier_call_event_handler(ISCD_CHARGE_DONE_EVENT);
		break;
	case POWER_NE_THIRDPLAT_CHARGING_RECHARGE:
		iscd_notifier_call_event_handler(ISCD_RECHARGE_EVENT);
		break;
	default:
		break;
...
static void iscd_cc_collect_work(struct work_struct *work)
{
	...
	if (!g_iscd_info || ret) {
		hwlog_err("get cc abnormal, stop collect work, ret = %d\n", ret);
		return;
	}
...
static int iscd_get_ocv_average(void)
{
	...
	for (i = 0; i < ISCD_OCV_SAMPLE_TIMES; i++) {
		ret = power_supply_get_int_property_value(POWER_PLATFORM_BAT_PSY_NAME,
			POWER_SUPPLY_PROP_VOLTAGE_NOW, &ocv_now);
		if (ret)
			return ret;
		if (ocv_now < ISCD_OCV_LOW_VALID_THRESHOLD || ocv_now > ISCD_OCV_HIGH_VALID_THRESHOLD) {
			flag++;
			msleep(ISCD_OCV_SAMPLE_INTERVAL);
			hwlog_err("get ocv_now invalid data=%d\n", ocv_now);
			continue;
		}
		ocv_sum += ocv_now;
		ocv_min = power_min_positive(ocv_now, ocv_min);
		ocv_max = power_max_positive(ocv_now, ocv_max);
		hwlog_info("ocv_now %d ocv_min %d ocv_max %d ocv_sum %d i %d\n",
			ocv_now, ocv_min, ocv_max, ocv_sum, i);
		if (i != ISCD_OCV_SAMPLE_TIMES - 1)
			msleep(ISCD_OCV_SAMPLE_INTERVAL);
	}

	if (flag > ISCD_OCV_EXCLUDED_TIMES)
		return 0;
	else
		return (ocv_sum - ocv_min - ocv_max) / (ISCD_OCV_SAMPLE_TIMES - ISCD_OCV_EXCLUDED_TIMES - flag);
}

static int iscd_ocv_collect_content_prepare(void)
{
	int ret;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	struct timespec64 tv = {0};
...
#endif
	ret = power_supply_get_int_property_value(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_TEMP, &g_iscd_info->iscd_psy_param.bat_temp);
	if (ret ||
		(g_iscd_info->iscd_psy_param.bat_temp / TENTH > ISCD_COLLECT_TEMP_LIMIT_HIGH) ||
		(g_iscd_info->iscd_psy_param.bat_temp / TENTH < ISCD_COLLECT_TEMP_LIMIT_LOW)) {
		hwlog_err("get bat temp invalid ret = %d\n", ret);
		return -EINVAL;
	}

	ret += power_supply_get_int_property_value(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_CYCLE_COUNT, &g_iscd_info->iscd_psy_param.bat_cycle);
	ret += power_supply_get_int_property_value(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_CURRENT_NOW, &g_iscd_info->iscd_psy_param.bat_cur);
	ret += power_supply_get_int_property_value(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN, &g_iscd_info->iscd_psy_param.bat_fcc_design);
	ret += power_supply_get_int_property_value(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_VOLTAGE_NOW, &g_iscd_info->iscd_psy_param.bat_vol);
	if (ret)
		return -EINVAL;
	...
	g_iscd_info->ocv_update_data.batt_chargecycles = g_iscd_info->iscd_psy_param.bat_cycle *
		ISCD_CYCLE_UPLOAD_FACTOR;
	g_iscd_info->ocv_update_data.cc_value += g_iscd_info->iscd_psy_param.bat_cur * delta_time *
		POWER_UA_PER_MA / (POWER_SEC_PER_MIN * POWER_MIN_PER_HOUR);
	g_iscd_info->ocv_update_data.tbatt = g_iscd_info->iscd_psy_param.bat_temp;
	...
	hwlog_info("get bat param from power supply, cycle %d, cur_now: %d, vol_now %d, \
		bat_temp %d, bat_fcc_design %d\n",
		g_iscd_info->iscd_psy_param.bat_cycle, g_iscd_info->iscd_psy_param.bat_cur,
		g_iscd_info->iscd_psy_param.bat_vol, g_iscd_info->iscd_psy_param.bat_temp,
		g_iscd_info->iscd_psy_param.bat_fcc_design);
	return 0;
}
...
static void iscd_ocv_collect_work(struct work_struct *work)
{
	...
	ret = iscd_ocv_collect_content_prepare();
	if (ret)
		goto try_next_time;

	if (g_iscd_info->iscd_state_machine != CHARGE_DONE_COLLECTING ||
		g_iscd_info->ocv_update_data.ocv_volt_uv < ISCD_OCV_LOW_VALID_THRESHOLD ||
		g_iscd_info->ocv_update_data.ocv_volt_uv > ISCD_OCV_HIGH_VALID_THRESHOLD ||
		g_iscd_info->iscd_psy_param.bat_cur > ISCD_CUR_VALID_THRESHOLD ||
		g_iscd_info->iscd_psy_param.bat_cur < -ISCD_CUR_VALID_THRESHOLD) {
		hwlog_err("quit charge done collecting, stop collect ocv. state:%d, cur:%d, ocv%d",
			g_iscd_info->iscd_state_machine, g_iscd_info->iscd_psy_param.bat_cur,
			g_iscd_info->ocv_update_data.ocv_volt_uv);
		iscd_shutdown_collecting();
		goto stop_work;
	}
	...
	struct device_attribute *attr, char *buff)
{
	...
	power_supply_get_int_property_value(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_CYCLE_COUNT, &data.bat_cyc);
...
static int iscd_power_event_register()
{
	int ret;
	...
	if (ret)
		goto fail_unregister_charging;

	g_iscd_info->thirdplat_event_nb.notifier_call = iscd_charge_thirdplat_event_notifier_call;
	ret = power_event_bnc_register(POWER_BNT_THIRDPLAT_CHARGING, &g_iscd_info->thirdplat_event_nb);
	if (ret)
		goto fail_unregister_thirdplat_chg;

	return 0;

fail_unregister_thirdplat_chg:
	power_event_bnc_unregister(POWER_BNT_CHARGING, &g_iscd_info->event_nb);
fail_unregister_charging:
...
static void iscd_sys_exit(struct bsoh_device *di)
{
	...
	kfree(g_dmd_content);
	power_event_bnc_unregister(POWER_BNT_THIRDPLAT_CHARGING, &g_iscd_info->thirdplat_event_nb);
	power_event_bnc_unregister(POWER_BNT_CHG, &g_iscd_info->chg_event_nb);
	power_event_bnc_unregister(POWER_BNT_CHARGING, &g_iscd_info->event_nb);
