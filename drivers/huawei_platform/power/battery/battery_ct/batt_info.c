...
int get_battery_group_sn(char *buf, int buf_size)
{
	uint64_t sn_hash = 0;
	int ret;

	if (buf_size < MAX_RAW_SN_LEN + 1) {
		hwlog_err("[%s] buff is too small\n", __func__);
		return -EOVERFLOW;
	}
	ret = batt_create_cur_group_sn(&sn_hash);
	if (ret)
		return ret;
	ret = snprintf_s(buf, MAX_RAW_SN_LEN + 1, MAX_RAW_SN_LEN, "%016llX", sn_hash);
	if (ret < 0)
		return ret;
	return 0;
}
