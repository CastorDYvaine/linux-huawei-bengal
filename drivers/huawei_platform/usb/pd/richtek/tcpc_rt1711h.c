...
static int rt1711_get_vid(struct tcpc_device *tcpc)
{
	struct rt1711_chip *chip = tcpc_get_dev_data(tcpc);
		...
		struct tcpc_device *tcpc_dev, bool en, int pull)
{
	...
	/* ETEK7304: Disable auto idle mode during lpm */
	if (chip->vendor_id == RICHTEK_1711_VID ||
		(chip->vendor_id == ETEK_ET7304_VID && chip->chip_id == ETEK_ET7304_DID)) {
