...
static int32_t sy3132cs_ln_fix_proc(int32_t data)
{
	...
	if (data > 0) {
		...
		factor = temp_log * nature_log_coef +
			nature_log_offset;
		temp_data = div_s64(data * factor, DATA_EXPAND_10000);
	} else {
		temp_data = 0;
	}
	return temp_data;
...
void sy3132cs_update_dark_value(void)
{
	...
	for (i = 0; i < (ALL_CHANNEL_NUM - 1); i++) {
		if (i < CH7_VALUE)
			...
			dark_value[CH7_VALUE] = sy3132cs_algo.dummyc_avg * sy3132cs_algo.dark_data.dark_ratio[CH7_VALUE] * 2 / DATA_EXPAND_100; // c8
		else
			dark_value[CH8_VALUE] = sy3132cs_algo.dummyc_avg * sy3132cs_algo.dark_data.dark_ratio[CH8_VALUE] * 2 / DATA_EXPAND_100 - IR_DARK_OFFSET; // nir
	}
	if (dark_value[CH8_VALUE] < 0)
		dark_value[CH8_VALUE] = 0;
...
void sy3132cs_normal_mode(struct sy3132cs_ctx *ctx)
{
	...
	sy3132cs_algo.avg_data[CH9_VALUE] =
		div_u64(sy3132cs_algo.avg_data[CH9_VALUE] * SY3132CS_ALS_IT_ST, sy3132cs_algo.avg_flk_gain) * GAIN_CALI_NUM; // clear
