...
// for user len, need pkt_header and context_num in ar_data_req_t at least
#define AR_MIN_USER_LEN   (sizeof(struct pkt_header) + sizeof(unsigned int))
	...
	size_t len, loff_t *ppos)
{
	...
	if (((ar_data_req_t *)user_data)->pkt.length + sizeof(struct pkt_header) < len) {
		hwlog_err("%s len too long\n", __func__);
		if (user_data != NULL)
			kfree(user_data);
		return len;
	}
	...
	if (((ar_data_req_t *)user_data)->pkt.length + sizeof(struct pkt_header) > len) {
		hwlog_err("%s user_input is too long\n", __func__);
		((ar_data_req_t *)user_data)->pkt.length = len - sizeof(struct pkt_header);
	}

	if (len >= AR_MIN_USER_LEN) {
