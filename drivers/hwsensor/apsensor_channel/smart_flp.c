	...
	size_t len, loff_t *ppos)
{
	...
	if( len < sizeof(pkt_subcmd_req_t) ) {
		hwlog_err("%s data too short\n", __func__);
		if (user_data != NULL)
			kfree(user_data);
		return len;
	}

	if ((len < sizeof(struct pkt_header)) || (((pkt_subcmd_req_t *)user_data)->hd.length + sizeof(struct pkt_header) < len)) {
	...
	if (((pkt_subcmd_req_t *)user_data)->hd.length + sizeof(struct pkt_header) > len) {
		...
		((pkt_subcmd_req_t *)user_data)->hd.length = len - sizeof(struct pkt_header);
	}
