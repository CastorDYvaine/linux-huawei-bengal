...
const struct bt_device bt_device_array[] = {
	...
	{ "BT_FM_QUALCOMM_WCN3988", "qcom_wcn3988" },
...
static char *get_bt_fm_device_name(void)
{
	...
	{
		if (strncmp(bt_fm_chip_type,bt_device_array[i].chip_type,strlen(bt_device_array[i].chip_type)) == 0)
		{
			break;
		}
	}
...
{
    const char *is_cn_and_ovs_share_boardid = NULL;

    is_cn_and_ovs_share_boardid = get_bt_is_cn_and_ovs_share_boardid();
    if (is_cn_and_ovs_share_boardid != NULL)
    {
        FMDBG("%s enter is_cn_and_ovs_share_boardid:%s;\n", __func__, is_cn_and_ovs_share_boardid);
        seq_printf(m, "%s", is_cn_and_ovs_share_boardid);
    }
    else
    {
...
static const struct file_operations bt_is_cn_and_ovs_share_boardid_proc_fops = {
    .owner    = THIS_MODULE,
    .open    = bt_is_cn_and_ovs_share_boardid_open,
    .read    = seq_read,
    .llseek    = seq_lseek,
    .release    = single_release,
};
...
static int fm_sinr_samples_proc_show(struct seq_file *m, void *v)
{
	...
	{
		sinr_samples = (unsigned int)g_sinr_samples;
	}
				...
				size_t count, loff_t *ppos)
{
	...
	ret = (int)(copy_from_user(fm_status, userbuf, count));
	if (ret) {
