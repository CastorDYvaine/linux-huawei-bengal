	...
	const char *buff, size_t len, loff_t *off)
{
	...
	aw_haptic->boost_off_flag = false;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void aw_haptic_get_bst_vol(struct aw_haptic *aw_haptic)
{
	uint8_t bst_vol = aw_haptic->vmax; // default 9V
	int ele = 1;
	int tmp = 0;
	int ret;

	if (aw_haptic->effect_mode == LONG_VIB_RAM_MODE) {
		aw_haptic->func->set_bst_vol(aw_haptic, bst_vol);
		return;
	}

	ret = power_proxy_get_bat_temperature(&tmp);
	if (ret == 0 && tmp <= 0) { // temperature less than 0
		aw_haptic->boost_off_flag = true;  // boost off
		aw_info("get temperature is %d\n", tmp);
		return;
	}

	ret = power_proxy_get_ui_capacity(&ele);
	if (ret < 0) {
		aw_info("power capacity get error");
		return;
	}
	aw_info("get ui capacity ele is %d", ele);

	switch (bat_model_get_bat_cathode_type()) {
	case BAT_MODEL_BAT_CATHODE_TYPE_GRAPHITE:
		aw_info("type is BAT_MODEL_BAT_CATHODE_TYPE_GRAPHITE");
		if (ele <= GRAPHITE_MAX_THERSHOLD && ele > GRAPHITE_MID_THERSHOLD)
			bst_vol = AW_HAPTIC_BST_MID_VOL;  // 7.5V
		else if (ele <= GRAPHITE_MID_THERSHOLD && ele > GRAPHITE_MIN_THERSHOLD)
			bst_vol = AW_HAPTIC_BST_MIN_VOL;  // 6V
		else if (ele <= GRAPHITE_MIN_THERSHOLD)
			aw_haptic->boost_off_flag = true;  // boost off
		else
			bst_vol = aw_haptic->vmax;  // default 9V
		break;
	case BAT_MODEL_BAT_CATHODE_TYPE_SILICON:
		aw_info("type is BAT_MODEL_BAT_CATHODE_TYPE_SILICON");
	...
	aw_info("get bst_vol is %d", bst_vol);
	aw_haptic->func->set_bst_vol(aw_haptic, bst_vol);
}
#endif
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (aw_haptic->info.is_enabled_lowpower_bst_config)
		aw_haptic_get_bst_vol(aw_haptic);
#endif
	if (aw_haptic->cust_boost_on == AW_SEL_BOOST_CFG_ON) {
		ram_vbat_comp(aw_haptic, false);
