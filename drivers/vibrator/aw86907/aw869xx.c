...
#define  HAPTIC_BST_MIN_VOL                     0x00
#define  HAPTIC_BST_MID_VOL                     0x13
...
#define  CAPACITY_MAX_THERSHOLD                 40
#define  CAPACITY_MID_THERSHOLD                 20

#define GRAPHITE_MAX_THERSHOLD                  30
#define GRAPHITE_MID_THERSHOLD                  20
#define GRAPHITE_MIN_THERSHOLD                  5
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void aw869xx_haptic_get_bst_vol(struct aw869xx *aw869xx)
{
	unsigned char bst_vol = HAPTIC_BST_VOL_9V;  // default 9V
	int ele = 1;
	int tmp = 0;
	int ret;

	if (aw869xx->effect_mode == LONG_VIB_RAM_MODE) {
		aw869xx_haptic_set_bst_vol(aw869xx, bst_vol);
		return;
	}

	ret = power_proxy_get_bat_temperature(&tmp);
	if (ret == 0 && tmp <= 0) { // temperature less than 0
		aw869xx->boost_off_flag = true;  // boost off
		pr_info("%s get temperature is %d\n", __func__, tmp);
		return;
	}

	ret = power_proxy_get_ui_capacity(&ele);
	if (ret < 0) {
		pr_info("%s : power capacity get error", __func__);
		return;
	}
	pr_info("%s get ui capacity ele is %d\n", __func__, ele);

	switch (bat_model_get_bat_cathode_type()) {
	case BAT_MODEL_BAT_CATHODE_TYPE_GRAPHITE:
		pr_info("%s type is BAT_MODEL_BAT_CATHODE_TYPE_GRAPHITE\n", __func__);
		if (ele <= GRAPHITE_MAX_THERSHOLD && ele > GRAPHITE_MID_THERSHOLD)
			bst_vol = HAPTIC_BST_MID_VOL;  // 7.5V
		else if (ele <= GRAPHITE_MID_THERSHOLD && ele > GRAPHITE_MIN_THERSHOLD)
			bst_vol = HAPTIC_BST_MIN_VOL;  // 6V
		else if (ele <= GRAPHITE_MIN_THERSHOLD)
			aw869xx->boost_off_flag = true;  // boost off
		else
			bst_vol = HAPTIC_BST_VOL_9V;  // default 9V
		break;
	case BAT_MODEL_BAT_CATHODE_TYPE_SILICON:
		pr_info("%s type is BAT_MODEL_BAT_CATHODE_TYPE_SILICON\n", __func__);
		if (ele <= CAPACITY_MAX_THERSHOLD && ele > CAPACITY_MID_THERSHOLD)
			bst_vol = HAPTIC_BST_MIN_VOL;  // 6V
		else if (ele <= CAPACITY_MID_THERSHOLD)
			aw869xx->boost_off_flag = true;  // boost off
		else
			bst_vol = HAPTIC_BST_VOL_9V;  // default 9V
		break;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (aw869xx->dts_info.is_enabled_lowpower_bst_config)
		aw869xx_haptic_get_bst_vol(aw869xx);
#endif
