	...
	struct Scsi_Host *host, enum blk_dump_scene dump_type)
{
	...
	/* record 10 latest time */
	for (i = 0; i < 10; i++)
		dev_err(hba->dev, "last_scsi_eh_time(%d): %lld",
			i, host->last_scsi_eh_time[i]);
	for (i = 0; i < 10; i++)
		dev_err(hba->dev, "last_ufs_eh_time(%d): %lld",
			i, host->last_ufs_eh_time[i]);

	/* record 6 autopm get/put interface */
	for (i = 0; i < 6; i++)
		dev_err(hba->dev, "last_autopm_time(%d): %lld",
			i, host->scsi_autopm_time[i]);
	for (i = 0; i < 6; i++)
		dev_err(hba->dev, "last_autopm_record(%d): %d",
			i, host->scsi_autopm_record[i]);
		...
		struct scsi_device *sdp, unsigned int timeout, bool eh_handle)
{
	...
	struct scsi_cmnd cmnd;
	unsigned char cmd[6] = {REQUEST_SENSE, 0, 0, 0, SCSI_SENSE_BUFFERSIZE, 0};
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,19,0))
	*pre_eol_info = buff[HEALTH_DESC_PARAM_EOL_INFO];
	*life_time_est_a = buff[HEALTH_DESC_PARAM_LIFE_TIME_EST_A];
	*life_time_est_b = buff[HEALTH_DESC_PARAM_LIFE_TIME_EST_B];
#else
...

{
	...
	/* enable nobarrier and disable flush reduce for diskmago */
	...
	if (!host->use_blk_mq)
		return;
#endif
	host->last_scsi_eh_cnt = 0;
	host->last_ufs_eh_cnt = 0;
