...
skip_unit_desc:
	...
	set_bootdevice_product_name(BOOT_DEVICE_UFS, dev_desc->model);
	...
	set_bootdevice_fw_version(BOOT_DEVICE_UFS, dev_desc->prl);
out:
	kfree(desc_buf);
	return err;
}
...
static void ufs_set_health_desc(struct ufs_hba *hba)
{
	int err;
	int buff_len = QUERY_DESC_HEALTH_DEF_SIZE;
	uint8_t desc_buf[QUERY_DESC_HEALTH_DEF_SIZE];
...
#if defined(CONFIG_MAS_ORDER_PRESERVE)
...
static void ufshcd_vreg_set_lpm(struct ufs_hba *hba)
{
	 ...
	 * buffer OR if bkops status is urgent for WB.
	 *
	 * ufs sleep is not powered off.
	 */
...
#ifdef CONFIG_QCOM_SCSI_UFS_DUMP
static void ufshcd_dump_scsi_command(struct ufs_hba *hba, unsigned int task_tag)
{
