...
static int adc_tm_probe(struct platform_device *pdev)
{
	...
	pr_err("gpio_num  is begin\n");
	gpio_num = of_get_named_gpio_flags(node, "NTC_boardID_change_gpio", 0, NULL);

	if (!gpio_is_valid(gpio_num)) {
		pr_err(" gpio: %s: gpio_num = %d is invalid\n", __func__, gpio_num);
		return 0;
	}

	ret = gpio_request(gpio_num, "NTC_boardID_change_gpio");
	if (ret)
		pr_err("%s, could not request NTC_boardID_change_gpio\n", __func__);

	ret = gpio_direction_output(gpio_num, 1);
	if (ret)
		pr_err("%s, gpio_num Could not set gpio direction\n", __func__);
