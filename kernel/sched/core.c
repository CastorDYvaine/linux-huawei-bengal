...
#define RT_THREAD_TIMEOUT_NS   3*1000000000
...
void scheduler_tick(void)
{
	...
	if (curr->sched_class == &rt_sched_class) {
		if ((rq->clock - curr->sched_info.last_arrival) >=
			(unsigned long long)RT_THREAD_TIMEOUT_NS)
			check_rt_timeout_migration(rq, curr);
		else
