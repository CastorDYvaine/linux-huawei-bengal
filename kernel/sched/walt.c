...
migrate_top_tasks(struct task_struct *p, struct rq *src_rq, struct rq *dst_rq)
{
	...
	int top_index;
#ifdef CONFIG_HW_TOP_TASK
	u32 curr_window = p->ravg.curr_load;
	u32 prev_window = p->ravg.prev_load;
#else
...
#ifdef CONFIG_HW_SCHED_PRED_LOAD
		rq->predl_window_start = sync_rq->window_start;
#endif
...
#ifdef CONFIG_HW_SCHED_PRED_LOAD
	if (predl_window_size != sched_ravg_window && p->ravg.predl > new)
		new = p->ravg.predl;
#endif
	if (old >= new)
		return;
...
#ifdef CONFIG_HW_TOP_TASK
	p->ravg.curr_load = p->ravg.load_avg;
	curr_window = p->ravg.curr_load;
#endif
				 ...
				 int event, u64 wallclock, u64 irqtime)
{
	...
	u32 old_curr_window = p->ravg.curr_load;
#else
...
#ifdef CONFIG_HW_TOP_TASK
		nr_full_window = div64_u64((window_start - mark_start), walt_ravg_window);
#endif
	}
...
#ifdef CONFIG_HW_CPU_FREQ_GOV_SCHEDUTIL_COMMON
	if (old_window_start != rq->window_start) {
		sugov_mark_util_change(cpu_of(rq), WALT_WINDOW_ROLLOVER);
...
void init_new_task_load(struct task_struct *p)
{
	...
	p->ravg.predl_sum = 0;
	for (i = 0; i < RAVG_HIST_SIZE_MAX; ++i)
		p->ravg.predl_sum_history[i] = init_load_windows;
	for (i = 0; i < NUM_BUSY_BUCKETS; ++i)
		p->ravg.predl_busy_buckets[i] = 0;
	memset(p->ravg.predl_prev_window_cpu, 0, NR_CPUS);
	memset(p->ravg.predl_curr_window_cpu, 0, NR_CPUS);
#endif
	p->misfit = false;
	p->unfilter = sysctl_sched_task_unfilter_period;
}
