...
static int rt_timeout_load_balance_cpu_stop(void *data)
{
	...
	unsigned int min_rt_nr_running = -1;
	cpumask_t search_cpus = CPU_MASK_NONE;
	int src_cpu = task_cpu(next_task);
	raw_spin_lock_irqsave(&busiest_rq->lock, flags);
	busiest_rq->rt_active_balance = 0;
	cpumask_and(&search_cpus, &next_task->cpus_allowed, cpu_coregroup_mask(src_cpu));
	for_each_cpu(cpu, &search_cpus) {
		if (cpu == src_cpu || !cpu_active(cpu) || cpu_isolated(cpu))
			continue;
		lowest_rq = cpu_rq(cpu);
		if (lowest_rq->rt.rt_nr_running < min_rt_nr_running) {
			dst_rq = lowest_rq;
			min_rt_nr_running = lowest_rq->rt.rt_nr_running;
		}
	}
	if (dst_rq == NULL) {
	...
	double_lock_balance(busiest_rq, dst_rq);
	deactivate_task(busiest_rq, next_task, 0);
	next_task->on_rq = TASK_ON_RQ_MIGRATING;
	set_task_cpu(next_task, dst_rq->cpu);
	next_task->on_rq = TASK_ON_RQ_QUEUED;
	activate_task(dst_rq, next_task, 0);
	resched_curr(dst_rq);
	double_unlock_balance(busiest_rq, dst_rq);
	put_task_struct(next_task);
	raw_spin_unlock_irqrestore(&busiest_rq->lock, flags);
	return 0;
}

void check_rt_timeout_migration(struct rq *rq, struct task_struct *p)
{
	...
	raw_spin_unlock(&rq->lock);
	if (need_actvie_lb)
		stop_one_cpu_nowait(task_cpu(p), rt_timeout_load_balance_cpu_stop,
