...
void f2fs_set_inode_flags(struct inode *inode)
{
	...
	unsigned int mask = S_SYNC|S_APPEND|S_IMMUTABLE|S_NOATIME|S_DIRSYNC|
				S_ENCRYPTED;

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 157))
	mask |= S_CASEFOLD|S_VERITY;
#endif
		...
		if (le64_to_cpu(ri->i_compr_blocks) >
			...
			f2fs_warn(sbi, "%s: inode (ino=%lx) has inconsistent "
				"i_compr_blocks:%llu, i_blocks:%llu, run fsck to fix",
				  ...
				  SECTOR_TO_BLOCK(inode->i_blocks));
#ifndef CONFIG_F2FS_FS_COMPRESSION_EX
			return false;
#endif
		}
...
#endif
#endif /* #ifdef CONFIG_F2FS_FS_DISK_TURBO */
	return true;
}
...
static int init_inner_inode(struct inode *inode)
{
	...
	if (unlikely(IS_ERR(inner))) {
		f2fs_err(sbi, "inode[%lu] iget inner ino[%lu] fail",
...
retry:
	...
	if (IS_ERR(node_page)) {
		...
		if (err == -ENOMEM) {
			...
			f2fs_err(sbi, "f2fs inode page read error! erro_num = %d", err);
...
void f2fs_evict_inode(struct inode *inode)
{
	...
	if (test_opt(sbi, COMPRESS_CACHE) && f2fs_is_compressed_inode(inode))
#else
...
retry:
	...
	if (unlikely(is_inode_flag_set(inode, FI_DIRTY_INODE))) {
		f2fs_inode_synced(inode);
		f2fs_warn(sbi, "correct inode status to avoid endless loop, since inode writeback flow is broken");
		f2fs_bug_on(sbi, 1);
	}
...
no_delete:
	...
	stat_sub_compr_blocks(inode, atomic_read(&F2FS_I(inode)->i_compr_blocks));
#endif
	...
	if (unlikely(is_inode_flag_set(inode, FI_DIRTY_INODE))) {
		...
		if (is_inode_flag_set(inode, FI_DIRTY_INODE)) {
			f2fs_inode_synced(inode);
			f2fs_warn(sbi, "inode info is inconsistent since inode evicts before we writeback it");
		}
		sb_end_intwrite(inode->i_sb);
		f2fs_bug_on(sbi, 1);
	}
