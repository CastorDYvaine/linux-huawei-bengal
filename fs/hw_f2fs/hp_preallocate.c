...
#define LOW_FREE_SPACE_THRESHOLD 10		/* 10% means the end of capacity */
#define SCALE_PERCENTAGE_TO_INTEGER 100
#define LOW_STORAGE_CAPACITY (128 * 1024)	/* 128GB capacity */
#define HIGH_STORAGE_CAPACITY (256 * 1024)	/* 256GB capacity */
// 10% storage threshold of 128GB:top-layer 1KB = 1000Byte
// 9135MB means 10% * 122GB(top-layer shows 128GB) - 3072MB(for create hyperhold file)
#define LOW_CAPACITY_THRESHOLD 9135
/* 10% storage threshold of 256GB */
#define HIGH_CAPACITY_THRESHOLD 21342
			...
			struct curseg_info *backupseg)
{
	...
	ret = memset_s(sum_footer, sizeof(struct summary_footer), 0, sizeof(struct summary_footer));
	if (ret != EOK)
		pr_err("[HP] %s: memset_s fail", __func__);
...
static int f2fs_hp_check_free_space(struct f2fs_sb_info *sbi)
{
	unsigned long long full_size =
		((u64)(sbi->user_block_count) * sbi->blocksize) / 1024 / 1024;
	unsigned long long free_size =
		((u64)(sbi->user_block_count - valid_user_blocks(sbi)) *
			sbi->blocksize) / 1024 / 1024;

	pr_err("[HP] disk free_size: %ld, fs full_size:%ld\n",
		free_size, (full_size * LOW_FREE_SPACE_THRESHOLD /
			SCALE_PERCENTAGE_TO_INTEGER));

	if (full_size < LOW_STORAGE_CAPACITY) {
		if (free_size < LOW_CAPACITY_THRESHOLD)
			return -ENOSPC;
	} else {
		if (free_size < HIGH_CAPACITY_THRESHOLD)
			return -ENOSPC;
	}
...
int f2fs_ioc_set_hp_file(struct file *filp, unsigned long arg)
{
	...
	ret = f2fs_hp_check_free_space(sbi);
	if (ret) {
		pr_err("[HP]do not have enough space for hp files! %d", ret);
		goto unlock_exit;
	}
