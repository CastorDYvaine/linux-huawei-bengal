...
bool is_ufs_support_tz_v2(struct f2fs_sb_info *sbi)
{
	...
	if (ret) {
		f2fs_err(sbi, "%s%s:failed to get ufs feature, ret = %d",
...
void init_f2fs_turbo_v2(struct f2fs_sb_info *sbi)
{
	...
	if (ret) {
		f2fs_warn(sbi, "%s%s:failed to get turbo status. ret = %d",
			TZ_LOG_TAG, __func__, ret);
		goto out;
	}
	...
	f2fs_info(sbi, "%s%s: turbo zone v2 is enabled", TZ_LOG_TAG, __func__);
out:
	kfree(status);
}
#endif
...
#if !defined(CONFIG_F2FS_FS_DISK_TURBO) || defined(CONFIG_LOG_EXCEPTION) && defined(CONFIG_F2FS_FS_DISK_TURBO)
void f2fs_tz_imonitor_send(struct f2fs_sb_info *sbi)
{
	...
	if (!obj) {
		f2fs_warn(sbi, "%s create eventobj failed", TZ_LOG_TAG);
		return;
	}
	...
	if (imonitor_send_event(obj) < 0)
		f2fs_warn(sbi, "%s send imonitor event failed", TZ_LOG_TAG);
	else
		f2fs_warn(sbi, "%s send imonitor event ok", TZ_LOG_TAG);
out:
	bd_mutex_unlock(&sbi->bd_mutex);
					...
					int tz_flag)
{
	...
	if (ret) {
		f2fs_warn(sbi, "%s%s:mnt_want_write_file err. ret %d",
			TZ_LOG_TAG, __func__, ret);
		return ret;
	}
...
int f2fs_ioc_set_turbo_return(struct file *filp, unsigned long arg)
{
	...
	if (!switchable && !allow_tz_switch(sbi)) {
		f2fs_warn(sbi, "Turbo zone tlc data is written, not allowed to switch");
		return -EPERM;
	}
...
int f2fs_get_tz_info(struct f2fs_sb_info *sbi)
{
	...
	if (!sbi->s_ndevs) {
		f2fs_warn(sbi, "%s%s:Single device is not supported in turbozone",
			TZ_LOG_TAG, __func__);
		return -EPERM;
	}
...
void init_f2fs_turbo_v1(struct f2fs_sb_info *sbi)
{
	...
	if (FDEV(F2FS_TURBO_DEV).start_blk < MAIN_BLKADDR(sbi)) {
		if (tz_info->total_segs <= FREE_I(sbi)->start_segno) {
			f2fs_warn(sbi, "%s%s:Turbo zone is too small",
				TZ_LOG_TAG, __func__);
...
int f2fs_ioc_migrate_file(struct file *filp, unsigned long arg)
{
	...
	if (is_inode_flag_set(inode, FI_COMPRESS_RELEASED)) {
		ret = f2fs_reserve_compress_blocks_internal(inode, MIGRATE_FILE);
		if (ret)
			goto out;
	}
#endif
	...
	if (tz_info->enabled)
		...
		ret = -EPERM;
#if defined(CONFIG_F2FS_FS_DEDUP) || defined(CONFIG_F2FS_FS_COMPRESSION_EX)
out:
#endif
	inode_unlock(inode);
...
int f2fs_ioc_set_tz_force_close(struct file *filp)
{
	...
	ret = blk_lld_tz_query(turbo_bdev, TZ_READ_DESC,
				(char *)status, F2FS_TZ_STATUS_SIZE);
	if (ret) {
		f2fs_err(sbi, "%s%s:failed to get turbo status.ret = %d",
			TZ_LOG_TAG, __func__, ret);
		ret = ret < 0 ? ret : -EAGAIN;
		goto out;
	}
	...
	if (!status->enable) {
		f2fs_warn(sbi, "%s%s:TZ is already disabled", TZ_LOG_TAG, __func__);
		ret = -1;
		goto out;
	}
	...
	if (ret) {
		f2fs_err(sbi, "%s%s:Failed to force to close TZ, ret %d",
			TZ_LOG_TAG, __func__, ret);
		...
		f2fs_info(sbi, "%s%s:Turbo zone is forced closed", TZ_LOG_TAG, __func__);
...
int f2fs_ioc_get_tz_space_info(struct file *filp, unsigned long arg)
{
	...
	if (!sbi->tz_info.enabled) {
		f2fs_warn(sbi, "TZ is already disabled");
		ret = -1;
		goto out;
	}
...
unsigned int f2fs_get_turbo_version(struct f2fs_sb_info *sbi)
{
	...
	if (f2fs_is_support_tz_v2(sbi)) {
		if (version)
			f2fs_warn(sbi, "Non-regular version, continue anyway");
...
int f2fs_query_tz_cap(struct f2fs_sb_info *sbi, struct tz_cap_info *cap_info)
{
	...
	if (ret) {
		f2fs_err(sbi, "%s%s:failed to get tz cap info, ret = %d",
			TZ_LOG_TAG, __func__, ret);
		ret = ret < 0 ? ret : -EAGAIN;
	}
			...
			block_t pblk, unsigned int len)
{
	...
	if (ret) {
		f2fs_err(sbi, "%s%s:failed to get tz blk info, ret = %d",
			TZ_LOG_TAG, __func__, ret);
		ret = ret < 0 ? ret : -EAGAIN;
	}
