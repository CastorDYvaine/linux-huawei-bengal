...
#include "node.h"
#include "segment.h"
#ifndef CONFIG_F2FS_FS_DISK_TURBO
#include "trace.h"
#endif
...
static int recover_orphan_inode(struct f2fs_sb_info *sbi, nid_t ino)
{
	...
	if (is_inode_flag_set(inode, FI_REVOKE_DEDUP)) {
		f2fs_notice(sbi, "recover orphan: ino[%lu] set revoke, flags[%lu]",
				ino, F2FS_I(inode)->flags);
	...
	if (is_inode_flag_set(inode, FI_DOING_DEDUP)) {
		f2fs_notice(sbi, "recover orphan: ino[%lu] set doing dedup, flags[%lu]",
				ino, F2FS_I(inode)->flags);
...
retry:
	cycles++;
	if (cycles % 10000 == 0)
		f2fs_err(sbi, "%s:%d: too many cycles(%lu), maybe an infinite loop!",
...
retry_flush_dents:
	...
	if (get_pages(sbi, F2FS_DIRTY_DENTS)) {
			...
			f2fs_err(sbi, "%s:%d: too many sync inode cycles(%lu), maybe an infinite loop!",
	...
	if (get_pages(sbi, F2FS_DIRTY_IMETA)) {
			...
			f2fs_err(sbi, "%s:%d: too many sync imeta cycles(%lu), maybe an infinite loop!",
...
retry_flush_nodes:
	...
	if (get_pages(sbi, F2FS_DIRTY_NODES)) {
			...
			f2fs_err(sbi, "%s:%d: too many sync nodes cycles(%lu), maybe an infinite loop!",
...
void f2fs_wait_on_all_pages(struct f2fs_sb_info *sbi, int type)
{
	...
	for (;;) {
		...
		count++;
		if ((ret == 0) && count >= HZ / DEFAULT_IO_TIMEOUT) {
			...
			f2fs_warn(sbi, "%s timeout writeback type=%d pages=%lld",
...
void f2fs_init_ino_entry_info(struct f2fs_sb_info *sbi)
{
	...
	sbi->max_orphans = (sbi->blocks_per_seg - F2FS_CP_PACKS -
			NR_CURSEG_PERSIST_TYPE - __cp_payload(sbi) - append) *
...
#endif /* end CONFIG_F2FS_FS_DISK_TURBO */
