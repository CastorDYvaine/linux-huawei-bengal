 ...
 *
 * 2) /sys/$s_id/features <deprecated>
 *   : shows on-disk features enabled by mkfs.f2fs, used for old kernels. This
 *     won't add new feature anymore, and thus, users should check entries in 3)
 *     instead of this 2).
 *
 * 3) /sys/$s_id/feature_list
...
#ifdef CONFIG_F2FS_FS_DEDUP
F2FS_FEATURE_RO_ATTR(hw_dedup, FEAT_HW_DEDUP);
#endif
#ifdef CONFIG_F2FS_FS_DISK_TURBO
...
#endif
F2FS_FEATURE_RO_ATTR(readonly, FEAT_READONLY);
#if defined(CONFIG_F2FS_FS_COMPRESSION) || defined(CONFIG_F2FS_FS_COMPRESSION_EX)
...
#ifdef CONFIG_DISK_MAGO
F2FS_FEATURE_RO_ATTR(disk_mago, FEAT_DISK_MAGO);
#endif
