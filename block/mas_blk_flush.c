...
static DEFINE_MUTEX(all_flush_q_lock);
...
void blk_power_off_flush(int emergency)
{
	...
	list_for_each_entry(mas_queue, &all_flush_q_list, flush_queue_node) {
		...
		if (blk_lld && blk_lld->flush_fn) {
			...
			bdev = bdget_disk(q->mas_queue.queue_disk, 0);
			if (!bdev) {
	...
	const void *data)
{
	...
	} else if (event == POWER_KEY_PRESS_RELEASE) {
