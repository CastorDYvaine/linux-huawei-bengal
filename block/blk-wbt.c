...
#ifndef CONFIG_BLK_CGROUP_IOSMART
...
static inline bool stat_sample_valid(struct blk_rq_stat *stat)
{
	 ...
	 */
#ifdef CONFIG_BLK_CGROUP_IOSMART
	return (stat[READ_FG].nr_samples >= 1 &&
		(stat[WRITE].nr_samples - stat[WRITE_FG].nr_samples)
...
#ifdef CONFIG_BLK_CGROUP_IOSMART
	if (stat[WRITE_FG].nr_samples &&
		stat[WRITE_FG].min > rwb->min_lat_nsec) {
...
{
	if ((bio->bi_opf & REQ_META) || (bio->bi_opf & REQ_SYNC))
		return false;
	switch (bio_op(bio)) {
	case REQ_OP_WRITE:
		/*
...
{
	enum wbt_flags flags = current_is_kswapd() ? WBT_KSWAPD : WBT_TRACKED;
	struct rq_wait *rqw = get_rq_wait(rwb, flags);
...
void wbt_issue(struct rq_qos *rqos, struct request *rq)
{
	 ...
	 */
#ifdef CONFIG_BLK_CGROUP_IOSMART
	if (!wbt_is_tracked(rq) && !rwb->sync_issue &&
		(rq->cmd_flags & REQ_FG)) {
#else
...
int wbt_init(struct request_queue *q)
{
	...
	buckets = 4;
#endif
	rwb->cb = blk_stat_alloc_callback(wb_timer_fn, wbt_data_dir, buckets, rwb);
	if (!rwb->cb) {
		kfree(rwb);
		return -ENOMEM;
	}
