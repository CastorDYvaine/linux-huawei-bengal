...
#define MAS_BLK_DUMP_BUF_LEN 1024UL
...
static int mas_blk_io_type_priv_flag(unsigned long flag, u64 mas_flag, char *type_buf, u32 len)
{
	int offset = 0;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	unsigned long priv_flag = mas_flag;
#else
	unsigned long priv_flag = flag;
#endif
...
static char *mas_blk_io_type(u64 op, unsigned int flag, u64 mas_flag, char *type_buf, int len)
{
	int offset = 0;
...
void mas_blk_dump_request(struct request *rq, enum blk_dump_scene s)
{
	...
	for (i = REQ_REQUEUE_IO_HW_LIMIT - 1; i < REQ_REQUEUE_MAX - 1; i++)
		count += snprintf(log + count, MAS_BLK_DUMP_BUF_LEN - count,
				"requeue_cnt%d %d tag: %d ",
				i, rq->mas_req.requeue_cnt[i], rq->tag);

	count += snprintf(log + count, MAS_BLK_DUMP_BUF_LEN - count,
			"hold_cnt: %u inflt_flag %x ",
			rq->hold_cnt, rq->inflt_flags);
