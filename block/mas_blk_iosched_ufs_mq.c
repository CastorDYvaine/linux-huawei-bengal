...
#define MAS_BLK_IO_GUARD_PERIOD_MS 5000
...
struct mas_ufs_mq_work {
	ktime_t last_queue_tm[10]; /* record 10 queue times */
	int last_run_cnt;
	ktime_t last_enter_tm;
	ktime_t last_exit_tm;
	unsigned int fail_exit;
	unsigned int exit_cnt;
...
struct mas_ufs_mq_async_sched {
	...
	bool (*async_sched_attempt_merge_fn)(struct bio *bio,
					     struct request_queue *q,
...
void mas_blk_latency_req_check_ufs(
	struct request *req, enum req_process_stage_enum req_stage)
{
	mas_blk_latency_req_check(req, req_stage);
	if (req_stage == REQ_PROC_STAGE_MQ_HOLD_START)
		req->hold_cnt++;
}

void mas_blk_queue_get_budget(struct request_queue *q)
{
	atomic_inc(&q->mas_queue.get_budget_cnt);
}

void mas_blk_queue_put_budget(struct request_queue *q)
{
	atomic_inc(&q->mas_queue.put_budget_cnt);
}
...
{
	pr_err("lst_in_t: %lld, lst_out_t: %lld, "
		"fail_exit: %lld, exit_cnt: %lld\n",
		work->last_enter_tm, work->last_exit_tm,
		work->fail_exit, work->exit_cnt);
}
...
void ufs_mq_dump_request(const struct request_queue *q, enum blk_dump_scene s)
{
	...
	pr_err("q get budget %d", atomic_read(&q->mas_queue.get_budget_cnt));
	pr_err("q put budget %d", atomic_read(&q->mas_queue.put_budget_cnt));
	...
	if (!list_empty(&ds_lld->sync_disp_list)) {
		...
		pr_err("last_run_cnt: %lld", sched_ds->sync_dispatch_work.last_run_cnt);
		for (i = 0; i < 10 ; i++)
			pr_err("last_queue_tm(%d): %lld",
				i, sched_ds->sync_dispatch_work.last_queue_tm[i]);
	}
	...
	struct request *rq, struct mas_ufs_sched_ds_lld *ds_lld)
{
	...
	if (ufs_mq_rq_is_highprio(rq)) {
		...
		if (ufs_mq_rq_is_fg(rq)) {
			...
			set_bit(REQ_INFLT_FG, &rq->inflt_flags);
		} else {
			atomic_inc(&ds_lld->vip_io_inflt_cnt);
			set_bit(REQ_INFLT_VIP, &rq->inflt_flags);
		}
#ifdef CONFIG_MAS_MQ_USING_CP
	...
	set_bit(REQ_INFLT_SYNC, &rq->inflt_flags);
	if (unlikely((atomic_inc_return(&ds_lld->sync_io_inflt_cnt) >
		...
		rq->mas_req.requeue_cnt[REQ_REQUEUE_IO_SW_LIMIT - 1]++;
	}
...
static int ufs_mq_async_io_limit(
	struct request *rq, struct mas_ufs_sched_ds_lld *ds_lld)
{
	set_bit(REQ_INFLT_ASYNC, &rq->inflt_flags);
	...
	const struct request *rq, struct mas_ufs_sched_ds_lld *ds_lld)
{
	...
	if (likely(rq->cmd_flags & REQ_SYNC)) {
		if (ufs_mq_rq_is_highprio((struct request *)rq)) {
			if (ufs_mq_rq_is_fg(rq)) {
				atomic_inc(&ds_lld->fg_io_inflt_cnt);
				set_bit(REQ_INFLT_FG,
					&(((struct request *)rq)->inflt_flags));
			} else {
				atomic_inc(&ds_lld->vip_io_inflt_cnt);
				set_bit(REQ_INFLT_VIP,
					&(((struct request *)rq)->inflt_flags));
			}
		} else {
			atomic_inc(&ds_lld->sync_io_inflt_cnt);
			set_bit(REQ_INFLT_SYNC,
				&(((struct request *)rq)->inflt_flags));
		}
	} else {
		atomic_inc(&ds_lld->async_io_inflt_cnt);
		set_bit(REQ_INFLT_ASYNC,
			&(((struct request *)rq)->inflt_flags));
	}
}
#endif
	...
	if (likely(rq->cmd_flags & REQ_SYNC)) {
		if (ufs_mq_rq_is_highprio((struct request *)rq)) {
			...
			if (ufs_mq_rq_is_fg(rq)) {
				...
				clear_bit(REQ_INFLT_FG,
					&(((struct request *)rq)->inflt_flags));
			} else {
				atomic_dec(&ds_lld->vip_io_inflt_cnt);
				clear_bit(REQ_INFLT_VIP,
					&(((struct request *)rq)->inflt_flags));
			}
#ifdef CONFIG_MAS_MQ_USING_CP
	...
	const struct mas_ufs_sched_ds_lld *ds_lld)
{
	...
	ret = BLK_STS_RESOURCE;
	if (!blk_mq_get_dispatch_budget((struct blk_mq_hw_ctx *)hctx)) {
		mas_blk_latency_req_check(bd->rq,
	...
	const struct request_queue *q)
{
	...
	if (unlikely(mas_blk_ft_mq_queue_rq_redirection(rq, (struct request_queue *)q)))
		return BLK_STS_OK;
#endif
	...
	rq->mas_req.requeue_cnt[REQ_REQUEUE_IO_HW_LIMIT - 1]++;
	if (ufs_mq_get_hp_inflt(ds_lld) +
		...
		rq->mas_req.requeue_cnt[REQ_REQUEUE_IO_HW_PENDING - 1]++;
	}
	...
	const struct request_queue *q, unsigned long delay_jiffies)
{
	...
	sched_ds->sync_dispatch_work.last_queue_tm[
		sched_ds->async_dispatch_work.last_run_cnt % 10] = ktime_get();
	sched_ds->sync_dispatch_work.last_run_cnt++;
}
...
static bool ufs_mq_attempt_merge(const struct request_queue *q,
				 const struct bio *bio, unsigned int nr_segs)
{
	unsigned long flags;
	struct mas_ufs_mq_sched *sched_ds =
		(struct mas_ufs_mq_sched *)(q->mas_queue.cust_queuedata);
				  ...
				  struct bio *bio, unsigned int nr_segs)
{
	...
	if (!bio_mergeable(bio) ||
		blk_rq_is_scsi(rq)) {
		ufs_mq_bio_to_request(rq, bio, nr_segs);
		spin_lock(&ctx->lock);
insert_rq:
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
			ret = bio_attempt_back_merge(rq, bio, nr_segs);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
			ret = bio_attempt_front_merge(rq, bio, nr_segs);
#else
...
static __always_inline bool ufs_mq_make_flush_request(unsigned int dispatch_op,
						      struct request *rq,
...
#endif
		ufs_mq_bio_to_request(rq, (struct bio *)bio, nr_segs);
		blk_insert_flush(rq);
		return true;
	}
	return false;
...
blk_qc_t ufs_mq_make_request(struct request_queue *q, struct bio *bio)
{
	...
	if (plug &&
	    ufs_mq_attempt_plug_merge(q, bio, plug, &same_queue_rq, nr_segs)) {
	...
	alloc_data.mas_cmd_flags = bio->mas_bi_opf;
	rq = blk_mq_get_request(q, bio, &alloc_data);
#else
	...
	if (likely(rq->cmd_flags & REQ_SYNC)) {
		ufs_mq_make_sync_request(bio, plug, rq, same_queue_rq,
					 &alloc_data, nr_segs);
		goto done;
	}
...
static void ufs_mq_sync_dispatch(const struct request_queue *q)
{
	...
	int ret;
	struct request *rq = NULL;
	struct request_queue *rq_q = NULL;
	struct blk_mq_hw_ctx *hctx = NULL;
	struct blk_mq_queue_data bd;
	struct mas_ufs_mq_sched *sched_ds =
		(struct mas_ufs_mq_sched *)(q->mas_queue.cust_queuedata);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
		hctx = blk_mq_map_queue(rq_q, rq->cmd_flags, rq->mq_ctx);
#else
		hctx = blk_mq_map_queue(rq_q, (int)rq->mq_ctx->cpu);
#endif
		bd.rq = rq;
		ret = ufs_mq_queue_rq(rq, hctx, ds_lld, &bd, rq_q);
		if (likely(ret == BLK_STS_OK)) {
			continue;
		} else if (ret == BLK_STS_RESOURCE) {
			__blk_mq_requeue_request(rq);
			ufs_mq_insert_sync_list(rq, rq_q);
			if (last_sync_io_compl_g_than(
				    ds_lld, ktime_get(), MAS_MQ_IO_JAM_MS) &&
				last_async_io_compl_g_than(
					ds_lld, ktime_get(), MAS_MQ_IO_JAM_MS))
				ufs_mq_run_delay_sync_list(rq_q, 1);
			else
				ufs_mq_run_delay_sync_list(rq_q, 0);
			sched_ds->sync_dispatch_work.fail_exit++;
			break;
		}
		blk_mq_end_request(rq, BLK_STS_IOERR);
	} while (1);

	sched_ds->sync_dispatch_work.exit_cnt++;
...
static bool ufs_mq_async_sched_fifo_attempt_merge_bio(struct bio *bio,
						      struct request_queue *q,
						      unsigned int nr_segs)
{
	struct request *rq = NULL;
	struct mas_ufs_mq_sched *sched_ds = (struct mas_ufs_mq_sched *)(q->mas_queue.cust_queuedata);
	struct mas_ufs_sched_ds_lld *ds_lld = sched_ds->sched_ds_lld;
	...
	const struct request_queue *q, unsigned long delay_jiffies)
{
	...
	sched_ds->async_dispatch_work.last_queue_tm[
		sched_ds->async_dispatch_work.last_run_cnt % 10] = ktime_get();
	sched_ds->async_dispatch_work.last_run_cnt++;
}
...
static void ufs_mq_async_dispatch(struct request_queue *q)
{
	...
	do {
		...
		rq_q = cur_req->q;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
		hctx = blk_mq_map_queue(rq_q, cur_req->cmd_flags, cur_req->mq_ctx);
#else
		hctx = blk_mq_map_queue(rq_q, (int)cur_req->mq_ctx->cpu);
#endif
		bd.rq = cur_req;
		
		ret = ufs_mq_queue_rq(cur_req, hctx, ds_lld, &bd, rq_q);
		if (likely(ret == BLK_STS_OK)) {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,4,0))
			...
			ufs_mq_async_requeue(cur_req, rq_q);
			ufs_mq_run_delay_async_list(rq_q, 1);
			break;
		}
...
extern void mas_blk_panic_dump(void);
	...
	struct request *rq, const struct request_queue *q, bool succ_done)
{
	...
	if (rq->mq_ctx && (rq->mq_ctx->cpu != raw_smp_processor_id()) &&
		...
		mas_blk_latency_req_check(rq, REQ_PROC_STAGE_COMPLETE_REMOTE);
		rq->csd.func = __cfi__ufs_mq_complete_request_remote;
		rq->csd.info = rq;
		rq->csd.flags = 0;
		smp_call_function_single_async((int)rq->mq_ctx->cpu, &rq->csd);
	} else {
		/* close softirq only for Qcom */
		mas_blk_latency_req_check(rq, REQ_PROC_STAGE_COMPLETE_CURRENT);
		rq->q->mq_ops->complete(rq);
	}
}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
	*hctx = blk_mq_map_queue(q, rq->cmd_flags, rq->mas_req.mq_ctx_generate);
#else
...
int ufs_mq_iosched_init(struct request_queue *q)
{
	...
	sched_ds->sync_dispatch_work.last_run_cnt = 0;

	sched_ds->async_dispatch_work.last_run_cnt = 0;

	atomic_set(&q->mas_queue.get_budget_cnt, 0);
	atomic_set(&q->mas_queue.put_budget_cnt, 0);
