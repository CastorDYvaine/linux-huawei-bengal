...
#ifdef CONFIG_IOSCHED_FMQ
	INIT_LIST_HEAD(&rl->list_sync);
	INIT_LIST_HEAD(&rl->list_async);
#endif
...
#ifdef CONFIG_IOSCHED_FMQ
	INIT_LIST_HEAD(&q->rl_head[0]);
	INIT_LIST_HEAD(&q->rl_head[1]);
#endif
		...
		req_flags_t rq_flags)
{
	struct request_queue *q = rl->q;
#ifdef CONFIG_IOSCHED_FMQ
	struct request_list *rl_temp = NULL;
	struct request_list *rl_next = NULL;
#endif
	q->nr_rqs[sync]--;
	rl->count[sync]--;
...
#ifdef CONFIG_IOSCHED_FMQ
	list_for_each_entry_safe(rl_temp, rl_next, &q->rl_head[0], list_async) {
		__freed_request(rl_temp, 0);
		list_del_init(&rl_temp->list_async);
	}
	list_for_each_entry_safe(rl_temp, rl_next, &q->rl_head[1], list_sync) {
		__freed_request(rl_temp, 1);
		list_del_init(&rl_temp->list_sync);
	}
#else
...
#ifdef CONFIG_IOSCHED_FMQ
	list_temp = is_sync ? &rl->list_sync : &rl->list_async;
#endif
retry:
...
#ifdef CONFIG_IOSCHED_FMQ
	if (rl->starved[is_sync] && list_empty(list_temp))
		list_add(list_temp, &q->rl_head[is_sync]);
#endif
